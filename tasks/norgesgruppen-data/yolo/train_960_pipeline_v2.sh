#!/usr/bin/env bash
# Diverse 960 pipeline — different seeds & augmentation for ensemble diversity
set -euo pipefail
cd "$(dirname "$0")/.."
...
echo "=== Stage 1/6: sweep_precision_v2 (35 epochs, from yolo26x.pt) ==="
uv run python yolo/train.py \
  ...
  --mixup 0.05 --copy-paste 0.05 --scale 0.4 --translate 0.08 --fliplr 0.5 \
  --seed 137 --run-tag v2_sweep --device $DEVICE

STAGE1=$(find runs -maxdepth 3 -path "*/v2_sweep_*/weights/best.pt" | sort | tail -1)
