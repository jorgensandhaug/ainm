...
def render_index_md(prep_overview: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Latest experiment report: [{Path('REP-0003-crop-embedder-comparison.md').name}]({DOCS_ROOT / 'reports' / 'REP-0003-crop-embedder-comparison.md'})",
        ...
        f"- YOLO dataset yaml: [{Path('dataset.yaml').name}]({Path(ROOT / 'data' / DATA_DATE / 'derived' / 'yolo' / 'dataset.yaml')})",
...
def render_data_dictionary_md() -> str:
    lines = [
        ...
        f"- [{Path('dataset.yaml').name}]({Path(ROOT / 'data' / DATA_DATE / 'derived' / 'yolo' / 'dataset.yaml')})",
...
def render_prep_playbook_md(
    ...
    lines = [
        ...
        "3. `python scripts/export_norgesgruppen_yolo.py`",
        "4. `python scripts/export_norgesgruppen_yolo.py --class-agnostic` when starting localization baselines",
        "5. `python scripts/benchmark_norgesgruppen_crops.py` when you need the current crop floor",
        "6. `python scripts/extract_norgesgruppen_gt_crops.py --split val --limit 100` only when you need actual crops on disk",
        "",
        "## What Is Canonical",
        "",
        ...
        f"- [{Path('REP-0003-crop-embedder-comparison.md').name}]({DOCS_ROOT / 'reports' / 'REP-0003-crop-embedder-comparison.md'})",
        f"- [{Path('category-strategy-manifest.json').name}]({CATEGORY_STRATEGY_MANIFEST_JSON})",
        f"- [{Path('image-sampling-manifest.json').name}]({IMAGE_SAMPLING_MANIFEST_JSON})",
        f"- [{Path('training-manifest.json').name}]({TRAINING_MANIFEST_JSON})",
...
def build_artifact_index() -> dict[str, Any]:
    return {
        "docs": {
            ...
            "latest_experiment_report_md": str((DOCS_ROOT / "reports" / "REP-0003-crop-embedder-comparison.md").relative_to(ROOT)),
        ...
        "data": {
            ...
            "yolo_dataset_yaml": str((ROOT / "data" / DATA_DATE / "derived" / "yolo" / "dataset.yaml").relative_to(ROOT)),
            "yolo_export_summary_json": str((ROOT / "data" / DATA_DATE / "derived" / "yolo" / "export-summary.json").relative_to(ROOT)),
