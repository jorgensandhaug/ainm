...
from norgesgruppen_prep_common import (
    ...
    DEEP_AUDIT_JSON,
    DOCS_ROOT,
    FIRST_CLASSIFIER_RECIPE_PLAN_MD,
...
def render_index_md(prep_overview: dict[str, Any]) -> str:
    lines = [
        ...
        f"- `./{Path('scripts/run_norgesgruppen_crop_python.sh')} {Path('scripts/cache_norgesgruppen_crop_classifier_embeddings.py')} --output-dir <dir>`",
        f"- `./{Path('scripts/run_norgesgruppen_crop_python.sh')} {Path('scripts/train_norgesgruppen_crop_classifier_cached.py')} --train-cache <train.pt> --val-cache <val.pt> --output-dir <dir>`",
        "",
        ...
        f"- Classifier/fusion strategy: [{Path(CLASSIFIER_FUSION_PLAN_MD.name).name}]({CLASSIFIER_FUSION_PLAN_MD})",
        f"- First classifier recipe: [{Path(FIRST_CLASSIFIER_RECIPE_PLAN_MD.name).name}]({FIRST_CLASSIFIER_RECIPE_PLAN_MD})",
        ...
        f"- Latest classifier-controls report: [{Path('REP-0010-first-classifier-controls.md').name}]({DOCS_ROOT / 'reports' / 'REP-0010-first-classifier-controls.md'})",
        f"- CPU-friendly classifier cache path: [{Path('cache_norgesgruppen_crop_classifier_embeddings.py').name}]({ROOT / 'scripts' / 'cache_norgesgruppen_crop_classifier_embeddings.py'}) and [{Path('train_norgesgruppen_crop_classifier_cached.py').name}]({ROOT / 'scripts' / 'train_norgesgruppen_crop_classifier_cached.py'})",
        f"- Latest pipeline report: [{Path('REP-0009-first-det-plus-retrieval-baseline.md').name}]({DOCS_ROOT / 'reports' / 'REP-0009-first-det-plus-retrieval-baseline.md'})",
        ...
        f"- Crop-classifier summary: [{Path('summary.json').name}]({CROP_CLASSIFIER_SUMMARY_JSON})",
        f"- Crop-classifier category roles: [{Path('category-role-manifest.json').name}]({CROP_CLASSIFIER_CATEGORY_ROLE_JSON})",
        f"- Crop-classifier train manifest: [{Path('train-all.jsonl').name}]({CROP_CLASSIFIER_TRAIN_JSONL})",
        f"- Crop-classifier val manifest: [{Path('val-all.jsonl').name}]({CROP_CLASSIFIER_VAL_JSONL})",
        f"- Crop-classifier val zero-train support manifest: [{Path('val-zero-train-support.jsonl').name}]({CROP_CLASSIFIER_VAL_ZERO_TRAIN_SUPPORT_JSONL})",
...
def render_prep_playbook_md(
    ...
    lines = [
        ...
        f"- Classifier/fusion next-step logic: [{Path(CLASSIFIER_FUSION_PLAN_MD.name).name}]({CLASSIFIER_FUSION_PLAN_MD})",
        f"- First classifier recipe: [{Path(FIRST_CLASSIFIER_RECIPE_PLAN_MD.name).name}]({FIRST_CLASSIFIER_RECIPE_PLAN_MD})",
        f"- Latest classifier-controls report: [{Path('REP-0010-first-classifier-controls.md').name}]({DOCS_ROOT / 'reports' / 'REP-0010-first-classifier-controls.md'})",
        f"- CPU-friendly classifier cache path: [{Path('cache_norgesgruppen_crop_classifier_embeddings.py').name}]({ROOT / 'scripts' / 'cache_norgesgruppen_crop_classifier_embeddings.py'}) and [{Path('train_norgesgruppen_crop_classifier_cached.py').name}]({ROOT / 'scripts' / 'train_norgesgruppen_crop_classifier_cached.py'})",
        f"- Latest pipeline report: [{Path('REP-0009-first-det-plus-retrieval-baseline.md').name}]({DOCS_ROOT / 'reports' / 'REP-0009-first-det-plus-retrieval-baseline.md'})",
        ...
        f"- [{Path('REP-0010-first-classifier-controls.md').name}]({DOCS_ROOT / 'reports' / 'REP-0010-first-classifier-controls.md'})",
...
def build_artifact_index() -> dict[str, Any]:
    return {
        "docs": {
            ...
            "classifier_fusion_plan_md": str(CLASSIFIER_FUSION_PLAN_MD.relative_to(ROOT)),
            "first_classifier_recipe_plan_md": str(FIRST_CLASSIFIER_RECIPE_PLAN_MD.relative_to(ROOT)),
            ...
            "latest_classifier_controls_report_md": str((DOCS_ROOT / "reports" / "REP-0010-first-classifier-controls.md").relative_to(ROOT)),
            "latest_pipeline_report_md": str((DOCS_ROOT / "reports" / "REP-0009-first-det-plus-retrieval-baseline.md").relative_to(ROOT)),
        ...
        "data": {
            ...
            "crop_classifier_summary_json": str(CROP_CLASSIFIER_SUMMARY_JSON.relative_to(ROOT)),
            "crop_classifier_category_role_json": str(CROP_CLASSIFIER_CATEGORY_ROLE_JSON.relative_to(ROOT)),
            "crop_classifier_train_jsonl": str(CROP_CLASSIFIER_TRAIN_JSONL.relative_to(ROOT)),
            "crop_classifier_val_jsonl": str(CROP_CLASSIFIER_VAL_JSONL.relative_to(ROOT)),
            "crop_classifier_val_train_seen_jsonl": str(CROP_CLASSIFIER_VAL_TRAIN_SEEN_JSONL.relative_to(ROOT)),
            "crop_classifier_val_zero_train_support_jsonl": str(CROP_CLASSIFIER_VAL_ZERO_TRAIN_SUPPORT_JSONL.relative_to(ROOT)),
        ...
        "scripts": {
            ...
            "crop_classifier_manifest_builder": str((ROOT / "scripts" / "build_norgesgruppen_crop_classifier_manifests.py").relative_to(ROOT)),
            "crop_classifier_common": str((ROOT / "scripts" / "norgesgruppen_crop_classifier_common.py").relative_to(ROOT)),
            "crop_classifier_cache_builder": str((ROOT / "scripts" / "cache_norgesgruppen_crop_classifier_embeddings.py").relative_to(ROOT)),
            "crop_classifier_trainer": str((ROOT / "scripts" / "train_norgesgruppen_crop_classifier.py").relative_to(ROOT)),
            "crop_classifier_cached_trainer": str((ROOT / "scripts" / "train_norgesgruppen_crop_classifier_cached.py").relative_to(ROOT)),
            "crop_classifier_evaluator": str((ROOT / "scripts" / "eval_norgesgruppen_crop_classifier.py").relative_to(ROOT)),
