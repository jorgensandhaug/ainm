"""GT-evidence model with observation-frequency blending.

Insight from Agent1: after the model produces predictions, blend in
the raw empirical class frequencies from observed cells with temperature
scaling. This simple post-processing adds ~0.5-1 points.

Also tests geometric mean blending (insight from Agent6).

Usage:
    uv run python scripts/agent4_gt_evidence_obsblend.py
"""
...
def run_gt_evidence_obsblend(
    *, name="agent4_gt_evidence_obsblend_v1",
    ...
    for hi, hr in enumerate(all_round_ids):
        ...
        # Train GT-evidence model (same as before)
        ...
        # Evaluate with observation-frequency blending
        ...
        for si, ar in sorted(analyses.items()):
            ...
            if items:
                ...
                ef = _build_evidence(eg, es, obs_mask)
            else:
                ef = _build_evidence([], None, np.zeros((h,w), dtype=bool))
            combined = np.concatenate([mf, ef], axis=-1)
            Xe = combined.reshape(-1, combined.shape[-1])
            model_probs = np.zeros((h*w, CLASS_COUNT))
            for c in range(CLASS_COUNT):
                model_probs[:,c] = np.clip(models[c].predict(Xe), 0, 1)
            model_probs /= model_probs.sum(axis=1, keepdims=True)
            model_pred = model_probs.reshape(h, w, CLASS_COUNT)

            # Observation-frequency blending (from Agent1)
            if items:
                obs_freq = np.zeros((h, w, CLASS_COUNT))
                obs_count = np.zeros((h, w))
                for rg, _ in items[:serve_ev]:
                    collapsed = collapse_internal_grid(rg)
                    for y in range(h):
                        for x in range(w):
                            if obs_mask[y, x]:
                                obs_freq[y, x, int(collapsed[y, x])] += 1.0
                                obs_count[y, x] += 1.0

                # Blend: weight = N / (N + temperature)
                blend_weight = obs_count / (obs_count + obs_blend_temperature)
                # Normalize obs_freq
                obs_freq_safe = obs_freq / np.maximum(obs_count[:,:,None], 1.0)
                # Final blend
                final = (1 - blend_weight[:,:,None]) * model_pred + blend_weight[:,:,None] * obs_freq_safe
            else:
                final = model_pred

            final = np.maximum(final, floor)
            final /= final.sum(axis=-1, keepdims=True)
            bd = score_prediction(gt, final)
    ...
    print(f"\n{'='*60}")
    print(f"GT-EVIDENCE+OBSBLEND: score={ms:.4f} kl={mk:.6f}")
    print(f"  serve_ev={serve_ev}, obs_temperature={obs_blend_temperature}")
    print(f"{'='*60}")

    od = paths.root / "data" / "artifacts" / "runs" / name
    od.mkdir(parents=True, exist_ok=True)
    (od / "results.json").write_text(json.dumps({
        "name": name, "serve_ev": serve_ev, "obs_blend_temperature": obs_blend_temperature,
...
if __name__ == "__main__":
    ...
    p.add_argument("--name", default="agent4_gt_evidence_obsblend_ev1_t20_v1")
    p.add_argument("--serve-ev", type=int, default=1)
    p.add_argument("--obs-temperature", type=float, default=20.0)
    a = p.parse_args()
    run_gt_evidence_obsblend(name=a.name, serve_ev=a.serve_ev, obs_blend_temperature=a.obs_temperature)
