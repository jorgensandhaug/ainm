"""Agent3's cellwise LightGBM with evidence replays + adaptive barren calibration.

Based on agent4's Evidence v2 approach but with:
1. Barren-round calibration from adaptive_ensemble
2. Forest boost for barren rounds
3. More aggressive hyperparameter tuning

Usage:
    uv run python scripts/agent3_cellwise_evidence_lgb.py --evidence-replays 15
"""
...
def build_map_features(grid: np.ndarray, settlements: list) -> np.ndarray:
    """Build rich per-cell map features (H, W, F)."""
    ...
    # Settlement/port maps
    sett_map = np.zeros((h, w), dtype=np.float64)
    port_map = np.zeros((h, w), dtype=np.float64)
    for s in settlements:
        y, x = (s.y if hasattr(s, 'y') else s['y']), (s.x if hasattr(s, 'x') else s['x'])
        sett_map[y, x] = 1.0
        has_port = s.has_port if hasattr(s, 'has_port') else s.get('has_port', False)
        if has_port:
            port_map[y, x] = 1.0
    features.extend([sett_map, port_map])

    # Neighborhood features at multiple scales
    ...
    # Distance features (simplified - using approximate BFS-free distances)
    ...
    # Settlement density at larger radii
    for radius in [4, 7]:
...
def build_evidence_features(
    ...
    if replay_grids:
        ...
        if replay_settlements_list:
            ...
            port_map_ev = np.zeros((h, w), dtype=np.float64)
            ...
            for settlements in replay_settlements_list:
                for s in settlements:
                    ...
                    if 0 <= sy < h and 0 <= sx < w:
                        ...
                        port_map_ev[sy, sx] += float(s.get("has_port", False))
            ...
            features.append(np.where(observed_mask, port_map_ev / safe_count, 0.0))
...
def load_replay_grids_and_settlements(
    replays_dir: Path,
    round_id: str,
    max_per_seed: int = 58,
...
def simulate_coverage_mask(h: int, w: int) -> np.ndarray:
...
def apply_barren_calibration(
    probs: np.ndarray,
    evidence_build_rate: float,
    barren_threshold: float = 0.03,
    settlement_scale: float = 0.3,
    ruin_scale: float = 0.2,
    forest_boost: float = 1.15,
) -> np.ndarray:
    """Apply adaptive_ensemble-style barren calibration on top of LGB predictions."""
    if evidence_build_rate < barren_threshold:
        probs = probs.copy()
        probs[:, :, 1] *= settlement_scale
        probs[:, :, 2] *= settlement_scale
        probs[:, :, 3] *= ruin_scale
        probs[:, :, 4] *= forest_boost
        probs[:, :, 0] = np.maximum(
            1.0 - probs[:, :, 1] - probs[:, :, 2] - probs[:, :, 3] - probs[:, :, 4] - probs[:, :, 5],
            0.01,
        )
        # Renormalize
...
def run_benchmark(
    *,
    name: str = "agent3_cellwise_evidence_lgb",
    n_estimators: int = 800,
    max_depth: int = 8,
    learning_rate: float = 0.02,
    max_replays_per_seed: int = 58,
    evidence_replays: int = 15,
    probability_floor: float = 0.008,
    barren_calibration: bool = True,
    n_jobs: int = 16,
) -> None:
    ...
    for held_out_idx, held_out_round in enumerate(all_round_ids):
        ...
        for seed_index, analysis_record in sorted(analyses.items()):
            ...
            # Apply barren calibration
            if barren_calibration:
                evidence_build_rate = float(
                    np.mean(pred[:, :, 1] + pred[:, :, 2] + pred[:, :, 3])
                )
                pred = apply_barren_calibration(pred, evidence_build_rate)
    ...
    print(f"\nCOMPARISON:")
    print(f"  This model:                      score={mean_score:.4f} kl={mean_kl:.6f}")
    print(f"  adaptive_ensemble_v17:           score=79.98  kl=0.075")
    print(f"  query_residual_v19:              score=76.89  kl=0.089")
    ...
    (output_dir / "results.json").write_text(json.dumps({
        ...
        "n_estimators": n_estimators,
        "max_depth": max_depth,
        "barren_calibration": barren_calibration,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", default="agent3_cellwise_evidence_lgb_v1")
    ...
    parser.add_argument("--probability-floor", type=float, default=0.008)
    parser.add_argument("--no-barren-calibration", action="store_true")
    ...
    run_benchmark(
        ...
        barren_calibration=not args.no_barren_calibration,
        n_jobs=args.n_jobs,
    )
