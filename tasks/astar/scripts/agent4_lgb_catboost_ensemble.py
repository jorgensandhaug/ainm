"""Geometric mean ensemble of LightGBM + CatBoost GT-evidence models.

Insight from Agent6: geometric mean in log-probability space matches KL scoring.

Usage:
    uv run python scripts/agent4_lgb_catboost_ensemble.py
"""
...
def run_ensemble(
    *, name="agent4_ensemble_lgb_cat_ev1_v1",
    ...
    for hi, hr in enumerate(all_round_ids):
        ...
        for si, ar in sorted(analyses.items()):
            ...
            lgb_probs = np.zeros((h*w, CLASS_COUNT))
            for c in range(CLASS_COUNT):
                lgb_probs[:,c] = np.clip(lgb_models[c].predict(Xe), floor, 1)
            lgb_probs /= lgb_probs.sum(axis=1, keepdims=True)

            # CatBoost predictions
            cat_probs = np.zeros((h*w, CLASS_COUNT))
            for c in range(CLASS_COUNT):
                cat_probs[:,c] = np.clip(cat_models[c].predict(Xe), floor, 1)
            cat_probs /= cat_probs.sum(axis=1, keepdims=True)

            # Geometric mean ensemble (in log space)
            log_lgb = np.log(np.maximum(lgb_probs, 1e-10))
            log_cat = np.log(np.maximum(cat_probs, 1e-10))
            log_blend = 0.5 * log_lgb + 0.5 * log_cat
    ...
    print(f"\n{'='*60}")
    print(f"LGB+CAT ENSEMBLE: score={ms:.4f} kl={mk:.6f} (serve_ev={serve_ev})")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    p.add_argument("--name", default="agent4_ensemble_lgb_cat_ev1_v1")
    p.add_argument("--serve-ev", type=int, default=1)
    a = p.parse_args()
    run_ensemble(name=a.name, serve_ev=a.serve_ev)
