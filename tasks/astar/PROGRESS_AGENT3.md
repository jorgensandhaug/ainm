...
466. Implemented + validated `round_heatmap_kernel_residual`:
   ...
   - validation result:
     - `3 passed`
467. Git checkpoint created + pushed for item 466:
   - commit:
     - `52f3372c`
   - message:
     - `agent3: add round heatmap kernel residual family`
468. Launch state for `round_heatmap_kernel_residual`:
   - machine headroom before launch:
     - available memory about `1.1 TiB`
   - corrected targeted-holdout gate sessions started:
     - `agent3_rheatkern_v1_gate`
     - `agent3_rheatkern_v2_gate`
     - `agent3_rheatkern_v3_gate`
     - `agent3_rheatkern_v4_gate`
   - command family:
     - `uv run python scripts/run_targeted_holdout_benchmark.py --model round_heatmap_kernel_residual_vX --held-out-round-id 36e581f1-73f8-453f-ab98-cbe3052b701b --held-out-round-id f1dac9a9-5cf1-49a9-8f17-d6cb5d5ba5cb --mode online_interactive --policy coverage --budget 50 --episode-seed 0 --with-png none --name agent3_round_heatmap_kernel_residual_vX_targeted_holdout_2rounds_corrected --jobs 1`
   - launch policy:
     - only `4` variants live in this new wave
     - `jobs=1` each
     - stopped there because shared-box headroom kept tightening while other agents ramped memory
469. New hypothesis branch started while all heatmap-state families run:
   - family:
     - `round_settlement_graph_factor_residual`
   - hypothesis:
     - cell heatmaps still compress away too much of the live settlement-state signal that replay and `simulate` expose
     - temporal settlement summaries, owner concentration, spacing graph statistics, and geometry sampled at observed settlements may infer round law better than coarse cell-count fields
     - a whole-round settlement-graph factor head could be the first branch that actually uses the richer settlement state rather than only class observations
   - objective:
     - test a settlement-state / interaction-graph family on top of base `v59/v60`
470. Implemented + validated `round_settlement_graph_factor_residual`:
   - new file:
     - `src/astar/student/predictor/round_settlement_graph_factor_residual.py`
   - reproducible models:
     - `round_settlement_graph_factor_residual`
   ...
   - variant mapping:
     ...
     - `v3/v4`: base `v59/v60`, samples `16`, rank `24`, ridge `2.0`, correction scale `1.00`
   - model form:
     - temporal settlement-state summaries + owner concentration + spacing graph stats + geometry at observed settlements -> low-rank residual bundle -> apply jointly across all seeds on top of base `v59/v60`
   - framework wiring:
     - `interactive.py`, `historical_benchmark.py`, `targeted_holdout_benchmark.py`, `model_eval.py`, and `cli.py`
   - validation command:
...
478. Implemented + validated `round_multiview_factor_residual`:
   ...
   - validation result:
     - `3 passed`
479. Git checkpoint created + pushed for item 478:
   - commit:
     - `bbea83ff`
   - message:
     - `agent3: add round multiview residual family`
480. Launch state for `round_multiview_factor_residual`:
   - machine headroom before launch:
     - available memory about `1.2 TiB`
   - observed shared-box pressure:
     - several foreign workers still above `100 GiB` RSS
     - `teacher_student_blend_v59` full corrected LOO still live with `4` heavy local workers
   - corrected targeted-holdout gate sessions started:
     - `agent3_mview_v1_gate`
     - `agent3_mview_v2_gate`
     - `agent3_mview_v3_gate`
     - `agent3_mview_v4_gate`
   - command family:
     - `uv run python scripts/run_targeted_holdout_benchmark.py --model round_multiview_factor_residual_vX --held-out-round-id 36e581f1-73f8-453f-ab98-cbe3052b701b --held-out-round-id f1dac9a9-5cf1-49a9-8f17-d6cb5d5ba5cb --mode online_interactive --policy coverage --budget 50 --episode-seed 0 --with-png none --name agent3_round_multiview_factor_residual_vX_targeted_holdout_2rounds_corrected --jobs 1`
   - launch policy:
     - only `4` variants live in this new wave
     - `jobs=1` each
     - no extra stacking beyond this bounded launch because the shared host still had large foreign jobs active
