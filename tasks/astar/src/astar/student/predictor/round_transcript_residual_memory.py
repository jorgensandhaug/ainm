...
ROUND_TRANSCRIPT_RESIDUAL_MEMORY_ALIAS = "round_transcript_residual_memory"
ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V1 = "round_transcript_residual_memory_v1"
ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V2 = "round_transcript_residual_memory_v2"
ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V3 = "round_transcript_residual_memory_v3"
ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V4 = "round_transcript_residual_memory_v4"
ROUND_TRANSCRIPT_RESIDUAL_MEMORY_MODEL_NAMES = frozenset(
    {
        ROUND_TRANSCRIPT_RESIDUAL_MEMORY_ALIAS,
...
ROUND_TRANSCRIPT_RESIDUAL_MEMORY_MODEL_CHOICE_LIST = [
    ROUND_TRANSCRIPT_RESIDUAL_MEMORY_ALIAS,
    ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V1,
    ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V2,
    ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V3,
...
class RoundTranscriptResidualMemoryVariantSpec(BaseModel):
...
class RoundTranscriptResidualMemoryCheckpoint(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
...
def is_round_transcript_residual_memory_model_name(model_name: str) -> bool:
    return model_name.strip().lower() in ROUND_TRANSCRIPT_RESIDUAL_MEMORY_MODEL_NAMES


def resolve_round_transcript_residual_memory_model_name(model_name: str) -> str:
    normalized = model_name.strip().lower()
    if normalized == ROUND_TRANSCRIPT_RESIDUAL_MEMORY_ALIAS:
        return ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V1
    if normalized in ROUND_TRANSCRIPT_RESIDUAL_MEMORY_MODEL_NAMES:
        return normalized
    raise ValueError(f"unsupported round_transcript_residual_memory model: {model_name}")


def resolve_round_transcript_residual_memory_variant_spec(
    model_name: str,
    *,
    samples_per_round: int | None = None,
) -> RoundTranscriptResidualMemoryVariantSpec:
    resolved = resolve_round_transcript_residual_memory_model_name(model_name)
    default_samples = {
        ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V1: 8,
        ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V2: 8,
        ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V3: 16,
        ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V4: 16,
    }[resolved]
    ...
    if resolved == ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V4:
        return RoundTranscriptResidualMemoryVariantSpec(
            model_name=resolved,
    ...
    if resolved == ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V3:
        return RoundTranscriptResidualMemoryVariantSpec(
            model_name=resolved,
    ...
    if resolved == ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V2:
        return RoundTranscriptResidualMemoryVariantSpec(
            model_name=resolved,
            base_model_name="teacher_student_blend_v60",
            samples_per_round=8,
...
def resolve_round_transcript_residual_memory_samples_per_round(
    model_name: str,
    *,
    samples_per_round: int | None = None,
) -> int:
    return resolve_round_transcript_residual_memory_variant_spec(
        model_name,
...
def _round_vector(
    ...
    *,
    ...
    pooled_mean = np.mean(np.stack(per_seed, axis=0), axis=0, dtype=np.float64)
    pooled_std = np.std(np.stack(per_seed, axis=0), axis=0, dtype=np.float64)
    return np.concatenate([pooled, pooled_mean, pooled_std], axis=0)


class RoundTranscriptResidualMemoryRoundPredictor(BaseRoundPredictor):
    model_config = ConfigDict(extra="forbid", arbitrary_types_allowed=True, frozen=True)

    name: str = ROUND_TRANSCRIPT_RESIDUAL_MEMORY_V1
...
def _fit_memory_predictor(
    ...
    *,
    spec: RoundTranscriptResidualMemoryVariantSpec,
    policy_name: str,
    round_ids: Sequence[str],
) -> RoundTranscriptResidualMemoryRoundPredictor:
    ...
    for row in index.iter_rows(named=True):
        ...
        residuals.append(np.asarray(target_bundle - base_bundle_tensor, dtype=np.float32))
    if not vectors or not residuals or residual_shape is None:
        raise ValueError("round_transcript_residual_memory produced no replay-backed training examples")
    return RoundTranscriptResidualMemoryRoundPredictor(
        name=spec.model_name,
        base_predictor=base_predictor,
        feature_vectors=np.asarray(vectors, dtype=np.float32),
        residual_bundles=np.asarray(residuals, dtype=np.float32),
...
def _save_checkpoint(
    predictor: RoundTranscriptResidualMemoryRoundPredictor,
    model_dir: Path,
    *,
    spec: RoundTranscriptResidualMemoryVariantSpec,
    policy_name: str,
    ...
    memory_path = model_dir / "round_memory.npz"
    np.savez_compressed(
        memory_path,
    ...
    checkpoint = RoundTranscriptResidualMemoryCheckpoint(
        name=predictor.name,
...
def _load_checkpoint(
    ...
    checkpoint = RoundTranscriptResidualMemoryCheckpoint.model_validate_json(
...
def load_or_fit_named_round_transcript_residual_memory_predictor(
    paths: WorkspacePaths,
    *,
    model_name: str,
    ...
    spec = resolve_round_transcript_residual_memory_variant_spec(
        model_name,
        samples_per_round=samples_per_round,
    )
    selected_round_ids = _selected_replay_round_ids(paths, round_ids)
...
__all__ = [
    ...
    "resolve_round_transcript_residual_memory_variant_spec",
]
