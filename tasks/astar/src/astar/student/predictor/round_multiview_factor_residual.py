...
ROUND_MULTIVIEW_FACTOR_RESIDUAL_ALIAS = "round_multiview_factor_residual"
ROUND_MULTIVIEW_FACTOR_RESIDUAL_V1 = "round_multiview_factor_residual_v1"
ROUND_MULTIVIEW_FACTOR_RESIDUAL_V2 = "round_multiview_factor_residual_v2"
ROUND_MULTIVIEW_FACTOR_RESIDUAL_V3 = "round_multiview_factor_residual_v3"
ROUND_MULTIVIEW_FACTOR_RESIDUAL_V4 = "round_multiview_factor_residual_v4"
ROUND_MULTIVIEW_FACTOR_RESIDUAL_MODEL_NAMES = frozenset(
    {
        ROUND_MULTIVIEW_FACTOR_RESIDUAL_ALIAS,
...
class RoundMultiviewFactorResidualVariantSpec(BaseModel):
...
class RoundMultiviewFactorResidualCheckpoint(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
...
def is_round_multiview_factor_residual_model_name(model_name: str) -> bool:
    return model_name.strip().lower() in ROUND_MULTIVIEW_FACTOR_RESIDUAL_MODEL_NAMES


def resolve_round_multiview_factor_residual_model_name(model_name: str) -> str:
    normalized = model_name.strip().lower()
    if normalized == ROUND_MULTIVIEW_FACTOR_RESIDUAL_ALIAS:
        return ROUND_MULTIVIEW_FACTOR_RESIDUAL_V1
    if normalized in ROUND_MULTIVIEW_FACTOR_RESIDUAL_MODEL_NAMES:
        return normalized
    raise ValueError(f"unsupported round_multiview_factor_residual model: {model_name}")


def resolve_round_multiview_factor_residual_variant_spec(
    model_name: str,
    *,
    samples_per_round: int | None = None,
) -> RoundMultiviewFactorResidualVariantSpec:
    resolved = resolve_round_multiview_factor_residual_model_name(model_name)
    default_samples = {
        ROUND_MULTIVIEW_FACTOR_RESIDUAL_V1: 8,
        ROUND_MULTIVIEW_FACTOR_RESIDUAL_V2: 8,
        ROUND_MULTIVIEW_FACTOR_RESIDUAL_V3: 16,
        ROUND_MULTIVIEW_FACTOR_RESIDUAL_V4: 16,
    }[resolved]
    ...
    if resolved == ROUND_MULTIVIEW_FACTOR_RESIDUAL_V4:
        return RoundMultiviewFactorResidualVariantSpec(
            model_name=resolved,
    ...
    if resolved == ROUND_MULTIVIEW_FACTOR_RESIDUAL_V3:
        return RoundMultiviewFactorResidualVariantSpec(
            model_name=resolved,
    ...
    if resolved == ROUND_MULTIVIEW_FACTOR_RESIDUAL_V2:
        return RoundMultiviewFactorResidualVariantSpec(
            model_name=resolved,
            base_model_name="teacher_student_blend_v60",
            samples_per_round=8,
...
def resolve_round_multiview_factor_residual_samples_per_round(
    model_name: str,
    *,
    samples_per_round: int | None = None,
) -> int:
    return resolve_round_multiview_factor_residual_variant_spec(
        model_name,
...
def _round_multiview_vector(context: LiveInferenceContext, *, max_queries: int) -> np.ndarray:
    ...
    settlement = _round_settlement_graph_vector(
        context.observations,
        context.geometry_bundle,
    )
    return np.concatenate([transcript, heatmap, settlement], axis=0)


class RoundMultiviewFactorResidualRoundPredictor(BaseRoundPredictor):
    model_config = ConfigDict(extra="forbid", arbitrary_types_allowed=True, frozen=True)

    name: str = ROUND_MULTIVIEW_FACTOR_RESIDUAL_V1
    ...
    def _predict_round_residual(self, context: LiveInferenceContext) -> np.ndarray:
        feature_vector = _round_multiview_vector(context, max_queries=self.max_queries)
...
def _fit_factor_predictor(
    ...
    *,
    spec: RoundMultiviewFactorResidualVariantSpec,
    policy_name: str,
    round_ids: Sequence[str],
) -> RoundMultiviewFactorResidualRoundPredictor:
    ...
    for row in index.iter_rows(named=True):
        ...
        if tuple(int(v) for v in target_bundle.shape) != residual_shape:
            continue
        x_rows.append(_round_multiview_vector(context, max_queries=spec.max_queries))
    ...
    if not x_rows or not y_rows or residual_shape is None:
        raise ValueError("round_multiview_factor_residual produced no replay-backed training examples")
    ...
    return RoundMultiviewFactorResidualRoundPredictor(
...
def _save_checkpoint(
    predictor: RoundMultiviewFactorResidualRoundPredictor,
    model_dir: Path,
    *,
    spec: RoundMultiviewFactorResidualVariantSpec,
    policy_name: str,
    ...
    memory_path = model_dir / "round_multiview_factor_residual.npz"
    np.savez_compressed(
        memory_path,
    ...
    checkpoint = RoundMultiviewFactorResidualCheckpoint(
        name=predictor.name,
...
def _load_checkpoint(
    ...
    checkpoint = RoundMultiviewFactorResidualCheckpoint.model_validate_json(
...
def load_or_fit_named_round_multiview_factor_residual_predictor(
    paths: WorkspacePaths,
    *,
    model_name: str,
