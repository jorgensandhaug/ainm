...
from astar.student.posterior.deepset_student import ObservationSetDistilledStudent
...
DEFAULT_HAZARD_POSTERIOR_V3_K = 5
DEFAULT_HAZARD_POSTERIOR_V3_RANK = 3
DEFAULT_HAZARD_POSTERIOR_V3_RIDGE_ALPHA = 8.0
DEFAULT_HAZARD_POSTERIOR_V3_MEAN_WEIGHT = 0.35
_HAZARD_V3_PREFIX = "hazard_posterior_v3_"


def hazard_posterior_v3_spec_for_model_name(
    model_name: str,
) -> tuple[int, int, float, float] | None:
    normalized = model_name.strip().lower()
    if normalized == "hazard_posterior_v3":
        return (
            DEFAULT_HAZARD_POSTERIOR_V3_K,
            DEFAULT_HAZARD_POSTERIOR_V3_RANK,
            DEFAULT_HAZARD_POSTERIOR_V3_RIDGE_ALPHA,
            DEFAULT_HAZARD_POSTERIOR_V3_MEAN_WEIGHT,
        )
    if not normalized.startswith(_HAZARD_V3_PREFIX):
        return None

    k_neighbors = DEFAULT_HAZARD_POSTERIOR_V3_K
    latent_rank = DEFAULT_HAZARD_POSTERIOR_V3_RANK
    ridge_alpha = DEFAULT_HAZARD_POSTERIOR_V3_RIDGE_ALPHA
    mean_weight = DEFAULT_HAZARD_POSTERIOR_V3_MEAN_WEIGHT
    for token in normalized.removeprefix(_HAZARD_V3_PREFIX).split("_"):
        if not token:
            continue
    ...
    if k_neighbors < 1 or latent_rank < 1 or ridge_alpha <= 0.0 or not (0.0 <= mean_weight <= 1.0):
        return None
    return (k_neighbors, latent_rank, ridge_alpha, mean_weight)


class HazardPosteriorV3Predictor(BaseRoundPredictor):
    model_config = ConfigDict(extra="forbid", arbitrary_types_allowed=True, frozen=True)

    name: str = "hazard_posterior_v3"
    student: ObservationSetDistilledStudent
    policy_name: str = "coverage"
    samples_per_round: int = Field(default=1, ge=1)
    k_neighbors: int = Field(default=DEFAULT_HAZARD_POSTERIOR_V3_K, ge=1)
    latent_rank: int = Field(default=DEFAULT_HAZARD_POSTERIOR_V3_RANK, ge=1)
    ridge_alpha: float = Field(default=DEFAULT_HAZARD_POSTERIOR_V3_RIDGE_ALPHA, gt=0.0)
    predicted_particle_weight: float = Field(
        default=DEFAULT_HAZARD_POSTERIOR_V3_MEAN_WEIGHT,
        ge=0.0,
        le=1.0,
    )
    ...
    def fit_from_workspace(
        ...
        *,
        ...
        k_neighbors: int = DEFAULT_HAZARD_POSTERIOR_V3_K,
        latent_rank: int = DEFAULT_HAZARD_POSTERIOR_V3_RANK,
        ridge_alpha: float = DEFAULT_HAZARD_POSTERIOR_V3_RIDGE_ALPHA,
        predicted_particle_weight: float = DEFAULT_HAZARD_POSTERIOR_V3_MEAN_WEIGHT,
        ...
        if not selected_round_ids:
            raise ValueError("hazard_posterior_v3 requires replay-backed training rounds")
        replay_episodes = [
        ...
        student = ObservationSetDistilledStudent.fit_from_dataset(
            dataset,
            teacher,
