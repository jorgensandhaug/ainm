...
ROUND_TRANSCRIPT_FACTOR_RESIDUAL_ALIAS = "round_transcript_factor_residual"
ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V1 = "round_transcript_factor_residual_v1"
ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V2 = "round_transcript_factor_residual_v2"
ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V3 = "round_transcript_factor_residual_v3"
ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V4 = "round_transcript_factor_residual_v4"
ROUND_TRANSCRIPT_FACTOR_RESIDUAL_MODEL_NAMES = frozenset(
    {
        ROUND_TRANSCRIPT_FACTOR_RESIDUAL_ALIAS,
...
ROUND_TRANSCRIPT_FACTOR_RESIDUAL_MODEL_CHOICE_LIST = [
    ROUND_TRANSCRIPT_FACTOR_RESIDUAL_ALIAS,
    ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V1,
    ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V2,
    ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V3,
...
class RoundTranscriptFactorResidualVariantSpec(BaseModel):
...
class RoundTranscriptFactorResidualCheckpoint(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
...
def is_round_transcript_factor_residual_model_name(model_name: str) -> bool:
    return model_name.strip().lower() in ROUND_TRANSCRIPT_FACTOR_RESIDUAL_MODEL_NAMES


def resolve_round_transcript_factor_residual_model_name(model_name: str) -> str:
    normalized = model_name.strip().lower()
    if normalized == ROUND_TRANSCRIPT_FACTOR_RESIDUAL_ALIAS:
        return ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V1
    if normalized in ROUND_TRANSCRIPT_FACTOR_RESIDUAL_MODEL_NAMES:
        return normalized
    raise ValueError(f"unsupported round_transcript_factor_residual model: {model_name}")


def resolve_round_transcript_factor_residual_variant_spec(
    model_name: str,
    *,
    samples_per_round: int | None = None,
) -> RoundTranscriptFactorResidualVariantSpec:
    resolved = resolve_round_transcript_factor_residual_model_name(model_name)
    default_samples = {
        ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V1: 8,
        ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V2: 8,
        ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V3: 16,
        ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V4: 16,
    }[resolved]
    ...
    if resolved == ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V4:
        return RoundTranscriptFactorResidualVariantSpec(
            model_name=resolved,
    ...
    if resolved == ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V3:
        return RoundTranscriptFactorResidualVariantSpec(
            model_name=resolved,
    ...
    if resolved == ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V2:
        return RoundTranscriptFactorResidualVariantSpec(
            model_name=resolved,
            base_model_name="teacher_student_blend_v60",
            samples_per_round=8,
...
def resolve_round_transcript_factor_residual_samples_per_round(
    model_name: str,
    *,
    samples_per_round: int | None = None,
) -> int:
    return resolve_round_transcript_factor_residual_variant_spec(
        model_name,
...
class RoundTranscriptFactorResidualRoundPredictor(BaseRoundPredictor):
    model_config = ConfigDict(extra="forbid", arbitrary_types_allowed=True, frozen=True)

    name: str = ROUND_TRANSCRIPT_FACTOR_RESIDUAL_V1
...
def _fit_factor_predictor(
    ...
    *,
    spec: RoundTranscriptFactorResidualVariantSpec,
    policy_name: str,
    round_ids: Sequence[str],
) -> RoundTranscriptFactorResidualRoundPredictor:
    ...
    if not x_rows or not y_rows or residual_shape is None:
        raise ValueError("round_transcript_factor_residual produced no replay-backed training examples")
    ...
    return RoundTranscriptFactorResidualRoundPredictor(
...
def _save_checkpoint(
    predictor: RoundTranscriptFactorResidualRoundPredictor,
    model_dir: Path,
    *,
    spec: RoundTranscriptFactorResidualVariantSpec,
    policy_name: str,
    ...
    memory_path = model_dir / "round_factor_residual.npz"
    np.savez_compressed(
        memory_path,
    ...
    checkpoint = RoundTranscriptFactorResidualCheckpoint(
        name=predictor.name,
...
def _load_checkpoint(
    ...
    checkpoint = RoundTranscriptFactorResidualCheckpoint.model_validate_json(
...
def load_or_fit_named_round_transcript_factor_residual_predictor(
    paths: WorkspacePaths,
    *,
    model_name: str,
    ...
    spec = resolve_round_transcript_factor_residual_variant_spec(
        model_name,
        samples_per_round=samples_per_round,
    )
    selected_round_ids = _selected_replay_round_ids(paths, round_ids)
...
__all__ = [
    ...
    "resolve_round_transcript_factor_residual_variant_spec",
]
