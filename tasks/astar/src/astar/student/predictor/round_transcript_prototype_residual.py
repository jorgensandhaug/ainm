...
ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_ALIAS = "round_transcript_prototype_residual"
ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V1 = "round_transcript_prototype_residual_v1"
ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V2 = "round_transcript_prototype_residual_v2"
ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V3 = "round_transcript_prototype_residual_v3"
ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V4 = "round_transcript_prototype_residual_v4"
ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_MODEL_NAMES = frozenset(
    {
        ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_ALIAS,
...
ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_MODEL_CHOICE_LIST = [
    ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_ALIAS,
    ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V1,
    ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V2,
    ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V3,
...
class RoundTranscriptPrototypeResidualVariantSpec(BaseModel):
...
class RoundTranscriptPrototypeResidualCheckpoint(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
...
def is_round_transcript_prototype_residual_model_name(model_name: str) -> bool:
    return model_name.strip().lower() in ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_MODEL_NAMES


def resolve_round_transcript_prototype_residual_model_name(model_name: str) -> str:
    normalized = model_name.strip().lower()
    if normalized == ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_ALIAS:
        return ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V1
    if normalized in ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_MODEL_NAMES:
        return normalized
    raise ValueError(f"unsupported round_transcript_prototype_residual model: {model_name}")


def resolve_round_transcript_prototype_residual_variant_spec(
    model_name: str,
    *,
    samples_per_round: int | None = None,
) -> RoundTranscriptPrototypeResidualVariantSpec:
    resolved = resolve_round_transcript_prototype_residual_model_name(model_name)
    default_samples = {
        ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V1: 8,
        ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V2: 8,
        ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V3: 16,
        ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V4: 16,
    }[resolved]
    ...
    if resolved == ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V4:
        return RoundTranscriptPrototypeResidualVariantSpec(
            model_name=resolved,
    ...
    if resolved == ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V3:
        return RoundTranscriptPrototypeResidualVariantSpec(
            model_name=resolved,
    ...
    if resolved == ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V2:
        return RoundTranscriptPrototypeResidualVariantSpec(
            model_name=resolved,
            base_model_name="teacher_student_blend_v60",
            samples_per_round=8,
...
def resolve_round_transcript_prototype_residual_samples_per_round(
    model_name: str,
    *,
    samples_per_round: int | None = None,
) -> int:
    return resolve_round_transcript_prototype_residual_variant_spec(
        model_name,
...
class RoundTranscriptPrototypeResidualRoundPredictor(BaseRoundPredictor):
    model_config = ConfigDict(extra="forbid", arbitrary_types_allowed=True, frozen=True)

    name: str = ROUND_TRANSCRIPT_PROTOTYPE_RESIDUAL_V1
    ...
    def _predict_round_residual(self, context: LiveInferenceContext) -> np.ndarray:
        ...
        distances = np.linalg.norm(self.centers - feature_vector[None, :], axis=1)
...
def _fit_prototype_predictor(
    ...
    *,
    spec: RoundTranscriptPrototypeResidualVariantSpec,
    policy_name: str,
    round_ids: Sequence[str],
) -> RoundTranscriptPrototypeResidualRoundPredictor:
    ...
    if not x_rows or not y_rows or residual_shape is None:
        raise ValueError("round_transcript_prototype_residual produced no replay-backed training examples")
    x_matrix = np.asarray(x_rows, dtype=np.float64)
    y_matrix = np.asarray(y_rows, dtype=np.float64)
    centers, assignments = _run_kmeans(x_matrix, k=spec.prototype_count)
    residual_prototypes = np.zeros((centers.shape[0], y_matrix.shape[1]), dtype=np.float64)
    for idx in range(centers.shape[0]):
        members = y_matrix[assignments == idx]
        if len(members) > 0:
            residual_prototypes[idx] = np.mean(members, axis=0, dtype=np.float64)
    return RoundTranscriptPrototypeResidualRoundPredictor(
...
def _save_checkpoint(
    predictor: RoundTranscriptPrototypeResidualRoundPredictor,
    model_dir: Path,
    *,
    spec: RoundTranscriptPrototypeResidualVariantSpec,
    policy_name: str,
    ...
    memory_path = model_dir / "round_prototype_residual.npz"
    np.savez_compressed(
        memory_path,
    ...
    checkpoint = RoundTranscriptPrototypeResidualCheckpoint(
        name=predictor.name,
...
def _load_checkpoint(
    ...
    checkpoint = RoundTranscriptPrototypeResidualCheckpoint.model_validate_json(
...
def load_or_fit_named_round_transcript_prototype_residual_predictor(
    paths: WorkspacePaths,
    *,
    model_name: str,
    ...
    spec = resolve_round_transcript_prototype_residual_variant_spec(
        model_name,
        samples_per_round=samples_per_round,
    )
    selected_round_ids = _selected_replay_round_ids(paths, round_ids)
...
__all__ = [
    ...
    "resolve_round_transcript_prototype_residual_variant_spec",
]
