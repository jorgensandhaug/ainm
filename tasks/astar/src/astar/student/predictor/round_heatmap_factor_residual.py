...
ROUND_HEATMAP_FACTOR_RESIDUAL_ALIAS = "round_heatmap_factor_residual"
ROUND_HEATMAP_FACTOR_RESIDUAL_V1 = "round_heatmap_factor_residual_v1"
ROUND_HEATMAP_FACTOR_RESIDUAL_V2 = "round_heatmap_factor_residual_v2"
ROUND_HEATMAP_FACTOR_RESIDUAL_V3 = "round_heatmap_factor_residual_v3"
ROUND_HEATMAP_FACTOR_RESIDUAL_V4 = "round_heatmap_factor_residual_v4"
ROUND_HEATMAP_FACTOR_RESIDUAL_MODEL_NAMES = frozenset(
    {
        ROUND_HEATMAP_FACTOR_RESIDUAL_ALIAS,
...
class RoundHeatmapFactorResidualVariantSpec(BaseModel):
...
class RoundHeatmapFactorResidualCheckpoint(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
...
def is_round_heatmap_factor_residual_model_name(model_name: str) -> bool:
    return model_name.strip().lower() in ROUND_HEATMAP_FACTOR_RESIDUAL_MODEL_NAMES


def resolve_round_heatmap_factor_residual_model_name(model_name: str) -> str:
    normalized = model_name.strip().lower()
    if normalized == ROUND_HEATMAP_FACTOR_RESIDUAL_ALIAS:
        return ROUND_HEATMAP_FACTOR_RESIDUAL_V1
    if normalized in ROUND_HEATMAP_FACTOR_RESIDUAL_MODEL_NAMES:
        return normalized
    raise ValueError(f"unsupported round_heatmap_factor_residual model: {model_name}")


def resolve_round_heatmap_factor_residual_variant_spec(
    model_name: str,
    *,
    samples_per_round: int | None = None,
) -> RoundHeatmapFactorResidualVariantSpec:
    resolved = resolve_round_heatmap_factor_residual_model_name(model_name)
    default_samples = {
        ROUND_HEATMAP_FACTOR_RESIDUAL_V1: 8,
        ROUND_HEATMAP_FACTOR_RESIDUAL_V2: 8,
        ROUND_HEATMAP_FACTOR_RESIDUAL_V3: 16,
        ROUND_HEATMAP_FACTOR_RESIDUAL_V4: 16,
    }[resolved]
    ...
    if resolved == ROUND_HEATMAP_FACTOR_RESIDUAL_V4:
        return RoundHeatmapFactorResidualVariantSpec(
            model_name=resolved,
    ...
    if resolved == ROUND_HEATMAP_FACTOR_RESIDUAL_V3:
        return RoundHeatmapFactorResidualVariantSpec(
            model_name=resolved,
    ...
    if resolved == ROUND_HEATMAP_FACTOR_RESIDUAL_V2:
        return RoundHeatmapFactorResidualVariantSpec(
            model_name=resolved,
            base_model_name="teacher_student_blend_v60",
            samples_per_round=8,
...
def resolve_round_heatmap_factor_residual_samples_per_round(
    model_name: str,
    *,
    samples_per_round: int | None = None,
) -> int:
    return resolve_round_heatmap_factor_residual_variant_spec(
        model_name,
...
def _round_heatmap_vector(evidence: RoundEvidenceBundle) -> np.ndarray:
    per_seed = []
    for seed_index in sorted(evidence.per_seed):
        ...
        coverage = coverage / max(float(np.max(coverage)), 1.0)
        counts = np.asarray(seed.observed_class_count_tensor, dtype=np.float64)
        count_mass = np.sum(counts, axis=-1)
        count_mass = count_mass / max(float(np.max(count_mass)), 1.0)
        class_freq = counts / np.clip(np.sum(counts, axis=-1, keepdims=True), 1.0, None)
        per_seed.append(
            np.concatenate(
                [
                    coverage.reshape(-1),
                    np.sqrt(count_mass).reshape(-1),
                    class_freq.reshape(-1),
                    np.asarray(seed.observed_class_frequencies, dtype=np.float64),
                    np.asarray(
                        [
                            float(seed.query_count) / 50.0,
                            float(seed.repeated_window_groups) / 10.0,
...
class RoundHeatmapFactorResidualRoundPredictor(BaseRoundPredictor):
    model_config = ConfigDict(extra="forbid", arbitrary_types_allowed=True, frozen=True)

    name: str = ROUND_HEATMAP_FACTOR_RESIDUAL_V1
...
def _fit_factor_predictor(
    ...
    *,
    spec: RoundHeatmapFactorResidualVariantSpec,
    policy_name: str,
    round_ids: Sequence[str],
) -> RoundHeatmapFactorResidualRoundPredictor:
    ...
    if not x_rows or not y_rows or residual_shape is None:
        raise ValueError("round_heatmap_factor_residual produced no replay-backed training examples")
    ...
    return RoundHeatmapFactorResidualRoundPredictor(
...
def _save_checkpoint(
    predictor: RoundHeatmapFactorResidualRoundPredictor,
    model_dir: Path,
    *,
    spec: RoundHeatmapFactorResidualVariantSpec,
    policy_name: str,
    ...
    memory_path = model_dir / "round_heatmap_factor_residual.npz"
    np.savez_compressed(
        memory_path,
    ...
    checkpoint = RoundHeatmapFactorResidualCheckpoint(
        name=predictor.name,
...
def _load_checkpoint(
    ...
    checkpoint = RoundHeatmapFactorResidualCheckpoint.model_validate_json(
...
def load_or_fit_named_round_heatmap_factor_residual_predictor(
    paths: WorkspacePaths,
    *,
    model_name: str,
    ...
    spec = resolve_round_heatmap_factor_residual_variant_spec(
        model_name,
        samples_per_round=samples_per_round,
    )
    selected_round_ids = _selected_replay_round_ids(paths, round_ids)
...
__all__ = [
    ...
    "resolve_round_heatmap_factor_residual_variant_spec",
]
