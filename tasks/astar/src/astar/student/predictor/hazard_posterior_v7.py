...
DEFAULT_HAZARD_POSTERIOR_V7_K = 5
DEFAULT_HAZARD_POSTERIOR_V7_RANK = 3
DEFAULT_HAZARD_POSTERIOR_V7_RIDGE_ALPHA = 32.0
DEFAULT_HAZARD_POSTERIOR_V7_MEAN_WEIGHT = 0.7
DEFAULT_HAZARD_POSTERIOR_V7_OBSERVATION_WEIGHT = 8.0
_HAZARD_V7_PREFIX = "hazard_posterior_v7_"


def hazard_posterior_v7_spec_for_model_name(
    model_name: str,
) -> tuple[int, int, float, float, float] | None:
    normalized = model_name.strip().lower()
    if normalized == "hazard_posterior_v7":
        return (
            DEFAULT_HAZARD_POSTERIOR_V7_K,
            DEFAULT_HAZARD_POSTERIOR_V7_RANK,
            DEFAULT_HAZARD_POSTERIOR_V7_RIDGE_ALPHA,
            DEFAULT_HAZARD_POSTERIOR_V7_MEAN_WEIGHT,
            DEFAULT_HAZARD_POSTERIOR_V7_OBSERVATION_WEIGHT,
        )
    if not normalized.startswith(_HAZARD_V7_PREFIX):
        return None

    k_neighbors = DEFAULT_HAZARD_POSTERIOR_V7_K
    latent_rank = DEFAULT_HAZARD_POSTERIOR_V7_RANK
    ridge_alpha = DEFAULT_HAZARD_POSTERIOR_V7_RIDGE_ALPHA
    mean_weight = DEFAULT_HAZARD_POSTERIOR_V7_MEAN_WEIGHT
    observation_weight = DEFAULT_HAZARD_POSTERIOR_V7_OBSERVATION_WEIGHT
    for token in normalized.removeprefix(_HAZARD_V7_PREFIX).split("_"):
        if not token:
            continue
...
class HazardPosteriorV7Predictor(BaseRoundPredictor):
    model_config = ConfigDict(extra="forbid", arbitrary_types_allowed=True, frozen=True)

    name: str = "hazard_posterior_v7"
    student: ObservationSetParticleRefinedStudent
    policy_name: str = "coverage"
    samples_per_round: int = Field(default=1, ge=1)
    k_neighbors: int = Field(default=DEFAULT_HAZARD_POSTERIOR_V7_K, ge=1)
    latent_rank: int = Field(default=DEFAULT_HAZARD_POSTERIOR_V7_RANK, ge=1)
    ridge_alpha: float = Field(default=DEFAULT_HAZARD_POSTERIOR_V7_RIDGE_ALPHA, gt=0.0)
    predicted_particle_weight: float = Field(
        default=DEFAULT_HAZARD_POSTERIOR_V7_MEAN_WEIGHT,
        ge=0.0,
        le=1.0,
    )
    observation_weight: float = Field(default=DEFAULT_HAZARD_POSTERIOR_V7_OBSERVATION_WEIGHT, gt=0.0)
    ...
    def fit_from_workspace(
        ...
        *,
        ...
        k_neighbors: int = DEFAULT_HAZARD_POSTERIOR_V7_K,
        latent_rank: int = DEFAULT_HAZARD_POSTERIOR_V7_RANK,
        ridge_alpha: float = DEFAULT_HAZARD_POSTERIOR_V7_RIDGE_ALPHA,
        predicted_particle_weight: float = DEFAULT_HAZARD_POSTERIOR_V7_MEAN_WEIGHT,
        observation_weight: float = DEFAULT_HAZARD_POSTERIOR_V7_OBSERVATION_WEIGHT,
        ...
        if not selected_round_ids:
            raise ValueError("hazard_posterior_v7 requires replay-backed training rounds")
        replay_episodes = [
