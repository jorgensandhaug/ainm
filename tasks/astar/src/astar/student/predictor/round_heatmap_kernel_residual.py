...
ROUND_HEATMAP_KERNEL_RESIDUAL_ALIAS = "round_heatmap_kernel_residual"
ROUND_HEATMAP_KERNEL_RESIDUAL_V1 = "round_heatmap_kernel_residual_v1"
ROUND_HEATMAP_KERNEL_RESIDUAL_V2 = "round_heatmap_kernel_residual_v2"
ROUND_HEATMAP_KERNEL_RESIDUAL_V3 = "round_heatmap_kernel_residual_v3"
ROUND_HEATMAP_KERNEL_RESIDUAL_V4 = "round_heatmap_kernel_residual_v4"
ROUND_HEATMAP_KERNEL_RESIDUAL_MODEL_NAMES = frozenset(
    {
        ROUND_HEATMAP_KERNEL_RESIDUAL_ALIAS,
...
class RoundHeatmapKernelResidualVariantSpec(BaseModel):
    ...
    length_scale: float = Field(gt=0.0)
    rank: int = Field(ge=1)
...
class RoundHeatmapKernelResidualCheckpoint(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
...
def is_round_heatmap_kernel_residual_model_name(model_name: str) -> bool:
    return model_name.strip().lower() in ROUND_HEATMAP_KERNEL_RESIDUAL_MODEL_NAMES


def resolve_round_heatmap_kernel_residual_model_name(model_name: str) -> str:
    normalized = model_name.strip().lower()
    if normalized == ROUND_HEATMAP_KERNEL_RESIDUAL_ALIAS:
        return ROUND_HEATMAP_KERNEL_RESIDUAL_V1
    if normalized in ROUND_HEATMAP_KERNEL_RESIDUAL_MODEL_NAMES:
        return normalized
    raise ValueError(f"unsupported round_heatmap_kernel_residual model: {model_name}")


def resolve_round_heatmap_kernel_residual_variant_spec(
    model_name: str,
    *,
    samples_per_round: int | None = None,
) -> RoundHeatmapKernelResidualVariantSpec:
    resolved = resolve_round_heatmap_kernel_residual_model_name(model_name)
    default_samples = {
        ROUND_HEATMAP_KERNEL_RESIDUAL_V1: 8,
        ROUND_HEATMAP_KERNEL_RESIDUAL_V2: 8,
        ROUND_HEATMAP_KERNEL_RESIDUAL_V3: 16,
        ROUND_HEATMAP_KERNEL_RESIDUAL_V4: 16,
    }[resolved]
    ...
    if resolved == ROUND_HEATMAP_KERNEL_RESIDUAL_V4:
        return RoundHeatmapKernelResidualVariantSpec(
            model_name=resolved,
    ...
    if resolved == ROUND_HEATMAP_KERNEL_RESIDUAL_V3:
        return RoundHeatmapKernelResidualVariantSpec(
            model_name=resolved,
    ...
    if resolved == ROUND_HEATMAP_KERNEL_RESIDUAL_V2:
        return RoundHeatmapKernelResidualVariantSpec(
            model_name=resolved,
            base_model_name="teacher_student_blend_v60",
            samples_per_round=8,
...
def resolve_round_heatmap_kernel_residual_samples_per_round(
    model_name: str,
    *,
    samples_per_round: int | None = None,
) -> int:
    return resolve_round_heatmap_kernel_residual_variant_spec(
        model_name,
...
def _rbf_feature_matrix(
    features: np.ndarray,
    anchors: np.ndarray,
    *,
    length_scale: float,
) -> np.ndarray:
    sqdist = np.sum((features[:, None, :] - anchors[None, :, :]) ** 2, axis=2, dtype=np.float64)
    return np.exp(-0.5 * sqdist / (length_scale**2))


class RoundHeatmapKernelResidualRoundPredictor(BaseRoundPredictor):
    model_config = ConfigDict(extra="forbid", arbitrary_types_allowed=True, frozen=True)

    name: str = ROUND_HEATMAP_KERNEL_RESIDUAL_V1
    ...
    def _predict_round_residual(self, context: LiveInferenceContext) -> np.ndarray:
        ...
        kernel_features = _rbf_feature_matrix(
            normalized[None, :],
            self.anchors,
            length_scale=self.length_scale,
        )[0]
        coeff = self.coeff_mean + (kernel_features @ self.weight_matrix)
...
def _fit_kernel_predictor(
    paths: WorkspacePaths,
    *,
    spec: RoundHeatmapKernelResidualVariantSpec,
    policy_name: str,
    round_ids: Sequence[str],
) -> RoundHeatmapKernelResidualRoundPredictor:
    ...
    if not x_rows or not y_rows or residual_shape is None:
        raise ValueError("round_heatmap_kernel_residual produced no replay-backed training examples")
    ...
    anchors, _ = _run_kmeans(normalized, k=spec.anchor_count)
    kernel_features = _rbf_feature_matrix(normalized, anchors, length_scale=spec.length_scale)
    ...
    return RoundHeatmapKernelResidualRoundPredictor(
...
def _save_checkpoint(
    predictor: RoundHeatmapKernelResidualRoundPredictor,
    model_dir: Path,
    *,
    spec: RoundHeatmapKernelResidualVariantSpec,
    policy_name: str,
    ...
    memory_path = model_dir / "round_heatmap_kernel_residual.npz"
    np.savez_compressed(
        memory_path,
        ...
        anchors=np.asarray(predictor.anchors, dtype=np.float32),
    ...
    checkpoint = RoundHeatmapKernelResidualCheckpoint(
        name=predictor.name,
...
def _load_checkpoint(
    ...
    checkpoint = RoundHeatmapKernelResidualCheckpoint.model_validate_json(
    ...
    return RoundHeatmapKernelResidualRoundPredictor(
        ...
        anchors=np.asarray(arrays["anchors"], dtype=np.float64),
        ...
        length_scale=checkpoint.length_scale,
        correction_scale=checkpoint.correction_scale,
        probability_floor=checkpoint.probability_floor,
    )


def load_or_fit_named_round_heatmap_kernel_residual_predictor(
    paths: WorkspacePaths,
    *,
    model_name: str,
    ...
    spec = resolve_round_heatmap_kernel_residual_variant_spec(
        model_name,
        samples_per_round=samples_per_round,
    )
    selected_round_ids = _selected_replay_round_ids(paths, round_ids)
    ...
    predictor = _fit_kernel_predictor(
        paths,
        spec=spec,
        policy_name=policy_name,
        round_ids=selected_round_ids,
...
__all__ = [
    ...
    "resolve_round_heatmap_kernel_residual_variant_spec",
]
