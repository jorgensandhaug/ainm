...
def _local_context(
    ...
    *,
    ...
    y0 = max(0, y - 1)
    y1 = min(previous_frame_grid.shape[0], y + 2)
    x0 = max(0, x - 1)
    x1 = min(previous_frame_grid.shape[1], x + 2)

    nearby_live_count = 0
    nearby_same_owner_count = 0
    nearby_other_owner_count = 0
    nearby_port_count = 0
    for settlement in previous_alive_settlements:
        if settlement.x == x and settlement.y == y:
            continue
        if not (x0 <= settlement.x < x1 and y0 <= settlement.y < y1):
            continue
        nearby_live_count += 1
        if settlement.has_port:
            nearby_port_count += 1
        if owner_id is not None and settlement.owner_id == owner_id:
            nearby_same_owner_count += 1
        else:
            nearby_other_owner_count += 1

    nearby_ruin_count = int(np.count_nonzero(previous_frame_grid[y0:y1, x0:x1] == 3))
    return (
        nearby_live_count,
...
def build_replay_measurement_bundle(
    ...
    site_opportunity_rows: list[dict[str, object]] = []
    settlement_rows: list[dict[str, object]] = []
    live_settlement_rows: list[dict[str, object]] = []
    ruin_rows: list[dict[str, object]] = []
    pairwise_rows: list[dict[str, object]] = []
    owner_year_rows: list[dict[str, object]] = []
    year_shock_rows: list[dict[str, object]] = []
    ...
    for run in runs:
        ...
        for step in range(len(run.frames) - 1):
            ...
            for x, y in positions:
                ...
                if prev_alive:
                    live_settlement_rows.append(dict(settlement_row))
            ...
            for owner_id, owner_settlements in sorted(previous_owner_groups.items()):
    ...
    live_settlement_transitions = _build_frame(
        live_settlement_rows,
...
def build_round_measurement_summary(
    ...
    return ReplayMeasurementRoundSummary(
        ...
        replay_run_count=sum(item.replay_run_count for item in ordered),
        frame_transition_count=sum(item.frame_transition_count for item in ordered),
        site_transition_count=sum(item.summary.site_transition_count for item in ordered),
        site_opportunity_count=sum(item.summary.site_opportunity_count for item in ordered),
        settlement_measurement_count=sum(
            item.summary.settlement_measurement_count for item in ordered
        ),
        live_settlement_transition_count=sum(
            item.summary.live_settlement_transition_count for item in ordered
        ),
        ruin_transition_count=sum(item.summary.ruin_transition_count for item in ordered),
        pairwise_candidate_count=sum(item.summary.pairwise_candidate_count for item in ordered),
        owner_year_count=sum(item.summary.owner_year_count for item in ordered),
        year_shock_count=sum(item.summary.year_shock_count for item in ordered),
        macro_trajectory_count=sum(item.summary.macro_trajectory_count for item in ordered),
