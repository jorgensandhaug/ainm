...
from astar.cli_output import (
    ...
    render_query_residual_online_audit,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    query_residual_online_audit_parser = subparsers.add_parser("run-query-residual-online-audit")
    query_residual_online_audit_parser.add_argument(
        "--model",
        choices=supported_query_residual_model_names(),
        required=True,
    )
    query_residual_online_audit_parser.add_argument("--round-id", action="append", default=None)
    query_residual_online_audit_parser.add_argument("--name", default="f1_query_residual_online_audit_v01")
    query_residual_online_audit_parser.add_argument("--policy", default="coverage")
    query_residual_online_audit_parser.add_argument("--budget", type=int, default=50)
    query_residual_online_audit_parser.add_argument("--episode-seed", type=int, default=0)
...
def _main() -> int:
    ...
    if args.command == "run-query-residual-online-audit":
        result = run_query_residual_online_audit(
            paths,
            model_name=args.model,
            round_ids=args.round_id,
            audit_name=args.name,
        ...
        _emit(args.json, result, render_query_residual_online_audit(result))
        return 0
