...
from astar.student.predictor.evidence_field import EVIDENCE_FIELD_MODEL_CHOICE_LIST
from astar.student.predictor.interactive import build_online_predictor
from astar.student.predictor.query_residual import QUERY_RESIDUAL_MODEL_NAMES
from astar.student.predictor.summary_bank import SUMMARY_BANK_MODEL_NAMES
from astar.student.predictor.transcript_memory import TRANSCRIPT_MEMORY_MODEL_CHOICE_LIST
...
def _model_choice_sort_key(name: str) -> tuple[str, int, str]:
    if "_v" in name:
        prefix, suffix = name.rsplit("_v", 1)
        if suffix.isdigit():
            return (prefix, int(suffix), "")
    return (name, -1, "")


ONLINE_MODEL_CHOICES = [
    "geometry_prior",
    "historical_bucket_prior",
    "latent_regime",
    *sorted(SUMMARY_BANK_MODEL_NAMES, key=_model_choice_sort_key),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    model_prediction_parser.add_argument(
        "--model",
        choices=PRIOR_AND_ONLINE_MODEL_CHOICES,
        required=True,
    )
