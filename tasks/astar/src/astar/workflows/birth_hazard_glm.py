...
class BirthHazardRoundMetric(BaseModel):
...
class BirthHazardCoefficient(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
...
class BirthHazardGlmAuditResult(BaseModel):
    ...
    coefficients: list[BirthHazardCoefficient]
    rounds: list[BirthHazardRoundMetric]
    artifact_path: Path
    report_path: Path
...
def _design_matrix(frame: pl.DataFrame) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    current_is_forest = (
        frame.get_column("current_class").to_numpy().astype(np.int64) == 4
    ).astype(np.float64)
    feature_arrays = [
        frame.get_column("coast").cast(pl.Float64).to_numpy(),
        frame.get_column("settlement_proximity").to_numpy(),
        frame.get_column("maritime_access").to_numpy(),
        frame.get_column("frontier_score").to_numpy(),
        frame.get_column("forest_density").to_numpy(),
        frame.get_column("mountain_density").to_numpy().astype(np.float64),
        frame.get_column("settlement_neighbors").to_numpy().astype(np.float64),
        frame.get_column("port_neighbors").to_numpy().astype(np.float64),
        frame.get_column("ruin_neighbors").to_numpy().astype(np.float64),
        frame.get_column("forest_neighbors").to_numpy().astype(np.float64),
        current_is_forest,
    ]
    features = np.stack(feature_arrays, axis=1).astype(np.float64)
    labels = frame.get_column("label").cast(pl.Float64).to_numpy()
    weights = frame.get_column("sample_weight").to_numpy().astype(np.float64)
    return features, labels, weights


def _render_report(result: BirthHazardGlmAuditResult) -> str:
    lines = [
...
def run_birth_hazard_glm_audit(
    ...
    *,
    ...
    if len(round_ids) < 2:
        raise ValueError("birth hazard glm audit requires at least two rounds")

    round_metrics: list[BirthHazardRoundMetric] = []
    ...
    for held_out_round_id in round_ids:
        ...
        train_x, train_y, train_w = _design_matrix(train)
        test_x, test_y, test_w = _design_matrix(test)
    ...
    full_x, full_y, full_w = _design_matrix(frame)
    ...
    coefficient_rows = [
        BirthHazardCoefficient(feature_name="intercept", coefficient=intercept),
        *[
            BirthHazardCoefficient(feature_name=name, coefficient=float(value))
            for name, value in zip(FEATURE_NAMES, coefficients, strict=True)
        ],
    ]
    artifact_dir = paths.artifacts_dir / "family1" / "hazard_glm" / audit_name
    artifact_dir.mkdir(parents=True, exist_ok=True)
    artifact_path = artifact_dir / "result.json"
    report_path = artifact_dir / "report.md"
    result = BirthHazardGlmAuditResult(
