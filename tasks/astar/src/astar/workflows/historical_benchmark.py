...
def run_historical_benchmark(
    ...
    *,
    ...
    if mode == "online_interactive" and is_transcript_sequence_residual_memory_model_name(normalized_model_name) and len(selected_round_ids) < 2:
        raise ValueError(
            "transcript_sequence_residual_memory requires at least two replay-backed analyzed rounds for holdout eval",
        )
    if mode == "online_interactive" and is_transcript_sequence_factor_residual_model_name(normalized_model_name) and len(selected_round_ids) < 2:
        raise ValueError(
            "transcript_sequence_factor_residual requires at least two replay-backed analyzed rounds for holdout eval",
        )
    if mode == "online_interactive" and is_round_transcript_residual_memory_model_name(normalized_model_name) and len(selected_round_ids) < 2:
        raise ValueError(
            "round_transcript_residual_memory requires at least two replay-backed analyzed rounds for holdout eval",
        )
    if mode == "online_interactive" and is_round_transcript_factor_residual_model_name(normalized_model_name) and len(selected_round_ids) < 2:
        raise ValueError(
            "round_transcript_factor_residual requires at least two replay-backed analyzed rounds for holdout eval",
        )
    if mode == "online_interactive" and is_round_transcript_prototype_residual_model_name(normalized_model_name) and len(selected_round_ids) < 2:
        raise ValueError(
            "round_transcript_prototype_residual requires at least two replay-backed analyzed rounds for holdout eval",
        )
    if mode == "online_interactive" and is_round_heatmap_factor_residual_model_name(normalized_model_name) and len(selected_round_ids) < 2:
        raise ValueError(
            "round_heatmap_factor_residual requires at least two replay-backed analyzed rounds for holdout eval",
        )
    if mode == "online_interactive" and is_round_heatmap_residual_memory_model_name(normalized_model_name) and len(selected_round_ids) < 2:
        raise ValueError(
            "round_heatmap_residual_memory requires at least two replay-backed analyzed rounds for holdout eval",
        )
    if mode == "online_interactive" and is_round_heatmap_prototype_residual_model_name(normalized_model_name) and len(selected_round_ids) < 2:
        raise ValueError(
            "round_heatmap_prototype_residual requires at least two replay-backed analyzed rounds for holdout eval",
        )
    if mode == "online_interactive" and is_round_heatmap_kernel_residual_model_name(normalized_model_name) and len(selected_round_ids) < 2:
        raise ValueError(
            "round_heatmap_kernel_residual requires at least two replay-backed analyzed rounds for holdout eval",
        )
    if mode == "online_interactive" and is_round_settlement_graph_factor_residual_model_name(normalized_model_name) and len(selected_round_ids) < 2:
        raise ValueError(
            "round_settlement_graph_factor_residual requires at least two replay-backed analyzed rounds for holdout eval",
        )
    if mode == "online_interactive" and is_round_multiview_factor_residual_model_name(normalized_model_name) and len(selected_round_ids) < 2:
        raise ValueError(
            "round_multiview_factor_residual requires at least two replay-backed analyzed rounds for holdout eval",
        )
    ...
    if (
        ...
        or is_transcript_sequence_residual_memory_model_name(normalized_model_name)
        or is_transcript_sequence_factor_residual_model_name(normalized_model_name)
        or is_round_transcript_residual_memory_model_name(normalized_model_name)
        or is_round_transcript_factor_residual_model_name(normalized_model_name)
        or is_round_transcript_prototype_residual_model_name(normalized_model_name)
        or is_round_heatmap_factor_residual_model_name(normalized_model_name)
        or is_round_heatmap_residual_memory_model_name(normalized_model_name)
        ...
        or is_spatial_correction_model_name(normalized_model_name)
        or is_adaptive_ensemble_model_name(normalized_model_name)
    ):
