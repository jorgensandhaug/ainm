            ...
            if is_query_residual_model_name(model_name)
                ...
                if is_summary_bank_model_name(model_name)
                    ...
                    if is_evidence_field_model_name(model_name)
                        ...
                        if is_transcript_memory_model_name(model_name)
                        else (
                            resolve_transcript_residual_memory_samples_per_round(
                                model_name,
                                samples_per_round=samples_per_round,
                            ...
                            if is_transcript_residual_memory_model_name(model_name)
                                ...
                                if is_transcript_sequence_residual_memory_model_name(model_name)
                                    ...
                                    if is_transcript_sequence_factor_residual_model_name(model_name)
                                    else (
                                        resolve_round_transcript_residual_memory_samples_per_round(
                                            model_name,
                                            samples_per_round=samples_per_round,
                                        )
                                        if is_round_transcript_residual_memory_model_name(model_name)
                                        else (
                                            resolve_round_transcript_factor_residual_samples_per_round(
                                                model_name,
                                                samples_per_round=samples_per_round,
                                            )
                                            if is_round_transcript_factor_residual_model_name(model_name)
                                            else (
                                                resolve_round_transcript_prototype_residual_samples_per_round(
                                                    model_name,
                                                    samples_per_round=samples_per_round,
                                                )
                                                if is_round_transcript_prototype_residual_model_name(model_name)
                                                else (
                                                    resolve_round_heatmap_factor_residual_samples_per_round(
                                                        model_name,
                                                        samples_per_round=samples_per_round,
                                                    )
                                                    if is_round_heatmap_factor_residual_model_name(model_name)
                                                    else (
                                                        resolve_round_heatmap_residual_memory_samples_per_round(
                                                            model_name,
                                                            samples_per_round=samples_per_round,
                                                        )
                                                        if is_round_heatmap_residual_memory_model_name(model_name)
                                                        else (
                                                            resolve_round_heatmap_prototype_residual_samples_per_round(
                                                                model_name,
                                                                samples_per_round=samples_per_round,
                                                            )
                                                            if is_round_heatmap_prototype_residual_model_name(model_name)
                                                            else (
                                                                resolve_round_heatmap_kernel_residual_samples_per_round(
                                                                    model_name,
                                                                    samples_per_round=samples_per_round,
                                                                )
                                                                if is_round_heatmap_kernel_residual_model_name(model_name)
                                                                else (
                                                                    resolve_round_settlement_graph_factor_residual_samples_per_round(
                                                                        model_name,
                                                                        samples_per_round=samples_per_round,
