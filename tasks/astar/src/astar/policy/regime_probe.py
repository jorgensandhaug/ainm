...
def _viewport_sum(viewport: Viewport, value_map: np.ndarray) -> float:
    window = np.asarray(
        value_map[
            viewport.y : viewport.y + viewport.h,
            viewport.x : viewport.x + viewport.w,
    ...
    return float(np.sum(window)) if window.size else 0.0
...
class PosteriorInformationPolicy(PosteriorDisagreementPolicy):
    model_config = ConfigDict(extra="forbid", arbitrary_types_allowed=True, frozen=True)

    name: str = "regime_probe_information_v1"
    expand_information_weight: float = Field(default=1.0, ge=0.0)
    expand_entropy_weight: float = Field(default=0.2, ge=0.0)
    expand_motif_weight: float = Field(default=0.15, ge=0.0)
    expand_neighbor_weight: float = Field(default=0.15, ge=0.0)
    repeat_information_weight: float = Field(default=1.1, ge=0.0)
    repeat_signal_weight: float = Field(default=0.4, ge=0.0)
    repeat_decay_penalty: float = Field(default=0.8, gt=0.0)
    ...
    def select(
        ...
        for seed_index in range(round_detail.seeds_count):
            ...
            for viewport in viewports:
                ...
                motif_score = motif_scores.get(key, 0.0)
                information_mass = _viewport_sum(viewport, disagreement_map)
                entropy_mass = _viewport_sum(viewport, predictive_entropy_map)
                neighbor_score = _neighbor_bonus(
                    viewport,
                    observations_by_seed.get(seed_index, []),
                    map_width=round_detail.map_width,
                    map_height=round_detail.map_height,
                ...
                if repeat_count == 0:
                    score = (
                        self.expand_information_weight * information_mass
                        + self.expand_entropy_weight * entropy_mass
                        + self.expand_motif_weight * motif_score
                        + self.expand_neighbor_weight * neighbor_score
                    )
                    tag = "regime_probe_information_expand"
                else:
                    repeat_signal = _window_repeat_signal(observations_by_key[key])
                    score = (
                        self.repeat_information_weight
                        * information_mass
                        / (1.0 + self.repeat_decay_penalty * float(repeat_count - 1))
                        + self.repeat_signal_weight * repeat_signal
                        + 0.05 * motif_score
                    )
                    tag = "regime_probe_information_repeat"
                scored.append(
                    (
                        -score,
                        repeat_count,
