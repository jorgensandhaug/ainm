...
def test_summary_rate_rollout_predictor_smoke(sample_paths: RepoPaths) -> None:
    ...
    predictor = SummaryRateRolloutPredictor.fit_from_workspace(
        ...
        samples_per_round=1,
        k_neighbors=1,
        model_name="summary_rate_rollout_predictor_test",
        synthetic_dataset_name="summary_rate_rollout_predictor_test_synth",
        rollout_years=5,
    )
...
def test_build_online_predictor_supports_summary_rate_rollout_models(sample_paths: RepoPaths) -> None:
    _duplicate_round_fixture(
        sample_paths,
        source_round_id=ROUND_ID,
        target_round_id=ROUND_ID_2,
    ...
    pure_adapter = build_online_predictor(
        "f1_summary_rate_rollout_birthcollapse_v01",
        paths=sample_paths,
        historical_round_ids=[ROUND_ID, ROUND_ID_2],
    )
    assert pure_adapter.name == "f1_summary_rate_rollout_birthcollapse_v01"

    blend_adapter = build_online_predictor(
