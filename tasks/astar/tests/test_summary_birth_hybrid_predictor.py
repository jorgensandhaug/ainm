...
def test_summary_birth_hybrid_predictor_smoke(sample_paths: RepoPaths) -> None:
    ...
    predictor = SummaryBirthHybridPredictor.fit_from_workspace(
        ...
        samples_per_round=1,
        k_neighbors=1,
        model_name="summary_birth_hybrid_predictor_test",
        synthetic_dataset_name="summary_birth_hybrid_predictor_test_synth",
    )
...
def test_build_online_predictor_supports_summary_birth_hybrid_models(sample_paths: RepoPaths) -> None:
    _duplicate_round_fixture(
        sample_paths,
        source_round_id=ROUND_ID,
        target_round_id=ROUND_ID_2,
    ...
    assert adapter.name == "f1_summary_birth_hybrid_t20_v01"
