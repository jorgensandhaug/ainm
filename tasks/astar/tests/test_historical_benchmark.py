...
def test_gbx_transcript_manifold_terminal_mapknn_online_historical_benchmark_runs(
    sample_paths: RepoPaths,
) -> None:
    _copy_round(sample_paths, ROUND_ID, TRAIN_ROUND_ID)
    _write_sample_analysis(sample_paths, round_id=ROUND_ID, seed_index=0)
    ...
    result = run_historical_benchmark(
        sample_paths,
        model_name="gbx_transcript_manifold_terminal_mapknn",
        round_ids=[ROUND_ID, TRAIN_ROUND_ID],
        mode="online_interactive",
        policy_name="coverage",
        samples_per_round=1,
        ...
        episode_seed=1,
        visualization_policy="none",
        benchmark_name="test_gbx_transcript_manifold_terminal_mapknn_online",
    )
    ...
    assert result.mode == "online_interactive"
    assert result.rounds[0].seed_results[0].model_name == "gbx_transcript_manifold_terminal_mapknn_v1"
    assert predictor.name == "gbx_transcript_manifold_terminal_mapknn_v1"
    assert predictor.predictor.selected_rank >= 1


def test_gbx_maponly_transcriptmanifold_mapknn_blend20_builds(
    sample_paths: RepoPaths,
) -> None:
    _copy_round(sample_paths, ROUND_ID, TRAIN_ROUND_ID)
    _write_sample_analysis(sample_paths, round_id=ROUND_ID, seed_index=0)
...
def test_gbx_transcript_regime_knn_terminal_mapknn_repeat_online_historical_benchmark_runs(
    sample_paths: RepoPaths,
) -> None:
    _copy_round(sample_paths, ROUND_ID, TRAIN_ROUND_ID)
    _write_sample_analysis(sample_paths, round_id=ROUND_ID, seed_index=0)
    ...
    result = run_historical_benchmark(
        sample_paths,
        model_name="gbx_transcript_regime_knn_terminal_mapknn_repeat",
        round_ids=[ROUND_ID, TRAIN_ROUND_ID],
        mode="online_interactive",
        policy_name="coverage",
        samples_per_round=1,
        ...
        episode_seed=1,
        visualization_policy="none",
        benchmark_name="test_gbx_transcript_regime_knn_terminal_mapknn_repeat_online",
    )
    ...
    assert result.mode == "online_interactive"
    assert result.rounds[0].seed_results[0].model_name == "gbx_transcript_regime_knn_terminal_mapknn_repeat_v1"
    assert predictor.name == "gbx_transcript_regime_knn_terminal_mapknn_repeat_v1"


def test_gbx_querylaw_roundbank_terminal_mapknn_online_historical_benchmark_runs(
    sample_paths: RepoPaths,
) -> None:
    _copy_round(sample_paths, ROUND_ID, TRAIN_ROUND_ID)
    _write_sample_analysis(sample_paths, round_id=ROUND_ID, seed_index=0)
    ...
    result = run_historical_benchmark(
        sample_paths,
        model_name="gbx_querylaw_roundbank_terminal_mapknn",
        round_ids=[ROUND_ID, TRAIN_ROUND_ID],
        mode="online_interactive",
        policy_name="coverage",
        samples_per_round=1,
        ...
        episode_seed=1,
        visualization_policy="none",
        benchmark_name="test_gbx_querylaw_roundbank_terminal_mapknn_online",
    )
    ...
    assert result.mode == "online_interactive"
    assert result.rounds[0].seed_results[0].model_name == "gbx_querylaw_roundbank_terminal_mapknn_v1"
    assert predictor.name == "gbx_querylaw_roundbank_terminal_mapknn_v1"


def test_gbx_queryknn_roundbank_terminal_mapknn_online_historical_benchmark_runs(
    sample_paths: RepoPaths,
) -> None:
    _copy_round(sample_paths, ROUND_ID, TRAIN_ROUND_ID)
    _write_sample_analysis(sample_paths, round_id=ROUND_ID, seed_index=0)
    ...
    result = run_historical_benchmark(
        sample_paths,
        model_name="gbx_queryknn_roundbank_terminal_mapknn",
        round_ids=[ROUND_ID, TRAIN_ROUND_ID],
        mode="online_interactive",
        policy_name="coverage",
        samples_per_round=1,
        ...
        episode_seed=1,
        visualization_policy="none",
        benchmark_name="test_gbx_queryknn_roundbank_terminal_mapknn_online",
    )
    ...
    assert result.mode == "online_interactive"
    assert result.rounds[0].seed_results[0].model_name == "gbx_queryknn_roundbank_terminal_mapknn_v1"
    assert predictor.name == "gbx_queryknn_roundbank_terminal_mapknn_v1"


def test_gbx_querymix_roundbank_terminal_mapknn_online_historical_benchmark_runs(
    sample_paths: RepoPaths,
) -> None:
    _copy_round(sample_paths, ROUND_ID, TRAIN_ROUND_ID)
    _write_sample_analysis(sample_paths, round_id=ROUND_ID, seed_index=0)
    ...
    result = run_historical_benchmark(
        sample_paths,
        model_name="gbx_querymix_roundbank_terminal_mapknn",
        round_ids=[ROUND_ID, TRAIN_ROUND_ID],
        mode="online_interactive",
        policy_name="coverage",
        samples_per_round=1,
        ...
        episode_seed=1,
        visualization_policy="none",
        benchmark_name="test_gbx_querymix_roundbank_terminal_mapknn_online",
    )
    ...
    assert result.mode == "online_interactive"
    assert result.rounds[0].seed_results[0].model_name == "gbx_querymix_roundbank_terminal_mapknn_v1"
    assert predictor.name == "gbx_querymix_roundbank_terminal_mapknn_v1"
...
def test_gbx_maponly_transcriptrepeat_blends_build(
    sample_paths: RepoPaths,
) -> None:
    _copy_round(sample_paths, ROUND_ID, TRAIN_ROUND_ID)
    _write_sample_analysis(sample_paths, round_id=ROUND_ID, seed_index=0)
...
def test_gbx_maponly_querylaw_roundbank_mapknn_blend20_builds(
    sample_paths: RepoPaths,
) -> None:
    _copy_round(sample_paths, ROUND_ID, TRAIN_ROUND_ID)
    _write_sample_analysis(sample_paths, round_id=ROUND_ID, seed_index=0)
    ...
    assert predictor.name == "gbx_maponly_querylaw_roundbank_mapknn_blend20_v1"
    assert predictor.predictor.transcript_weight == 0.2


def test_gbx_maponly_queryknn_roundbank_mapknn_blend20_builds(
    sample_paths: RepoPaths,
) -> None:
    _copy_round(sample_paths, ROUND_ID, TRAIN_ROUND_ID)
    _write_sample_analysis(sample_paths, round_id=ROUND_ID, seed_index=0)
    ...
    assert predictor.name == "gbx_maponly_queryknn_roundbank_mapknn_blend20_v1"
    assert predictor.predictor.transcript_weight == 0.2


def test_gbx_maponly_querymix_roundbank_mapknn_blend20_builds(
    sample_paths: RepoPaths,
) -> None:
    _copy_round(sample_paths, ROUND_ID, TRAIN_ROUND_ID)
    _write_sample_analysis(sample_paths, round_id=ROUND_ID, seed_index=0)
    ...
    assert predictor.name == "gbx_maponly_querymix_roundbank_mapknn_blend20_v1"
    assert predictor.predictor.transcript_weight == 0.2
...
def test_gbx_maponly_transcriptregime_manifold_ensemble_builds(
    sample_paths: RepoPaths,
) -> None:
    _copy_round(sample_paths, ROUND_ID, TRAIN_ROUND_ID)
    _write_sample_analysis(sample_paths, round_id=ROUND_ID, seed_index=0)
    ...
    expected_weights = {
        "gbx_maponly_transcriptregime_manifold_mapknn_blend20": (0.1, 0.1),
        "gbx_maponly_transcriptregime_manifolddelta_mapknn_blend20": (0.1, 0.1),
        "gbx_maponly_transcriptregime_manifoldtriple_mapknn_blend20": (0.1, 0.05, 0.05),
    }
    for model_name, expected in expected_weights.items():
        predictor = build_online_predictor(
            model_name,
            paths=sample_paths,
            historical_round_ids=[TRAIN_ROUND_ID],
        ...
        assert tuple(predictor.predictor.transcript_weights) == expected
