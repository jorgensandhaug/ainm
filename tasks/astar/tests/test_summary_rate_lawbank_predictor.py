...
def test_summary_rate_lawbank_predictor_smoke(sample_paths: RepoPaths) -> None:
    ...
    predictor = SummaryRateLawBankPredictor.fit_from_workspace(
        ...
        samples_per_round=1,
        k_neighbors=1,
        model_name="summary_rate_lawbank_predictor_test",
        synthetic_dataset_name="summary_rate_lawbank_predictor_test_synth",
    )
...
def test_build_online_predictor_supports_summary_rate_lawbank_models(sample_paths: RepoPaths) -> None:
    _duplicate_round_fixture(
        sample_paths,
        source_round_id=ROUND_ID,
        target_round_id=ROUND_ID_2,
    ...
    plain_adapter = build_online_predictor(
        "f1_summary_rate_lawbank_collapse_portsplit_v01",
        paths=sample_paths,
        historical_round_ids=[ROUND_ID, ROUND_ID_2],
    )
    assert plain_adapter.name == "f1_summary_rate_lawbank_collapse_portsplit_v01"

    teacher_adapter = build_online_predictor(
        "f1_summary_rate_lawbank_collapse_portsplit_teacher_v01",
        paths=sample_paths,
        historical_round_ids=[ROUND_ID, ROUND_ID_2],
    )
    assert teacher_adapter.name == "f1_summary_rate_lawbank_collapse_portsplit_teacher_v01"
    ...
    assert stress_adapter.name == "f1_summary_rate_lawbank_collapse_portsplit_teacher_stress_v01"


def test_cli_parser_accepts_summary_rate_lawbank_models() -> None:
    parser = build_parser()
    ...
    assert parsed.model == "f1_summary_rate_lawbank_collapse_portsplit_teacher_v01"
