...
def test_evaluate_round_behavioral_fingerprint_summary_returns_finite_metrics(
    ...
    report = evaluate_round_behavioral_fingerprint_summary(
        round_id=ROUND_ID,
    ...
    assert not any(name.startswith("year_shock::") for name in report.probe_summary_names)
    assert not any(name.startswith("macro::") for name in report.probe_summary_names)
    assert np.all(np.isfinite(np.asarray(report.probe_summary_std, dtype=np.float64)))
    assert any(metric.value is not None for metric in report.live_binary_metrics)
    assert any(metric.sample_count > 0 for metric in report.live_linear_metrics)
    assert len(report.ruin_binary_metrics) == 5
    assert any(metric.sample_count > 0 for metric in report.owner_linear_metrics)
    assert len(report.probe_support) == 5
    assert any(item.sample_count > 0 for item in report.probe_support)


def test_evaluate_behavioral_fingerprint_summary_uses_cached_measurements(
    sample_paths: RepoPaths,
) -> None:
    _write_replays_for_all_seeds(sample_paths, run_count=2)
    ...
    result = evaluate_behavioral_fingerprint_summary(
        sample_paths,
        round_ids=[ROUND_ID],
        max_holdout_runs=3,
        bootstrap_samples=2,
        rng_seed=0,
        name="behavioral_fingerprint_validation_cached_test",
    )

    assert result.report_count == 1
    assert result.artifact_path.exists()
    assert result.report_path.exists()
    assert result.mean_live_binary_brier is not None
    assert result.round_reports[0].round_id == ROUND_ID


def test_evaluate_behavioral_fingerprint_summary_profile_defaults(
    sample_paths: RepoPaths,
) -> None:
    _write_replays_for_all_seeds(sample_paths, run_count=2)
    summarize_round_replays(sample_paths, ROUND_ID)
    ...
    result = evaluate_behavioral_fingerprint_summary(
        ...
        rng_seed=0,
        name="behavioral_fingerprint_validation_profile_smoke_test",
    )
    ...
    assert result.live_max_rows == 8_000
    assert result.ruin_max_rows == 8_000
    assert result.pairwise_max_rows == 16_000
    assert result.owner_max_rows == 8_000
    assert result.elapsed_seconds >= 0.0


def test_evaluate_behavioral_fingerprint_summary_explicit_overrides_win(
    sample_paths: RepoPaths,
) -> None:
    _write_replays_for_all_seeds(sample_paths, run_count=2)
    summarize_round_replays(sample_paths, ROUND_ID)
    ...
    result = evaluate_behavioral_fingerprint_summary(
        ...
        ruin_max_rows=11_000,
        pairwise_max_rows=12_000,
        owner_max_rows=13_000,
        rng_seed=0,
        name="behavioral_fingerprint_validation_profile_override_test",
    )
    ...
    assert result.live_max_rows == 10_000
    assert result.ruin_max_rows == 11_000
    assert result.pairwise_max_rows == 12_000
    assert result.owner_max_rows == 13_000


def test_evaluate_behavioral_fingerprint_summary_default_name_depends_on_config(
    sample_paths: RepoPaths,
) -> None:
    _write_replays_for_all_seeds(sample_paths, run_count=2)
    summarize_round_replays(sample_paths, ROUND_ID)

    smoke_result = evaluate_behavioral_fingerprint_summary(
        sample_paths,
        round_ids=[ROUND_ID],
        validation_profile="smoke",
        rng_seed=0,
    )
    dev_result = evaluate_behavioral_fingerprint_summary(
        sample_paths,
        round_ids=[ROUND_ID],
