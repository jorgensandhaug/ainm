...
def test_summary_bank_decoder_predictor_smoke(sample_paths: RepoPaths) -> None:
    ...
    predictor = SummaryBankDecoderPredictor.fit_from_workspace(
        ...
        samples_per_round=1,
        k_neighbors=1,
        model_name="summary_bank_decoder_predictor_test",
        synthetic_dataset_name="summary_bank_decoder_predictor_test_synth",
    )
...
def test_build_online_predictor_supports_summary_bank_decoder_models(sample_paths: RepoPaths) -> None:
    _duplicate_round_fixture(
        sample_paths,
        source_round_id=ROUND_ID,
        target_round_id=ROUND_ID_2,
    ...
    assert adapter.name == "f1_summary_bank_decoder_v01"


def test_cli_parser_accepts_summary_bank_decoder_models() -> None:
    parser = build_parser()
    ...
    assert parsed.model == "f1_summary_bank_decoder_v01"
    ...
    assert parsed_teacher.model == "f1_summary_bank_decoder_teacher_v01"
    ...
    assert parsed_teacher_r3.model == "f1_summary_bank_decoder_teacher_r3_v02"
