...
const supplierName = `Minimal Proof Supplier ${suffix} AS`;
const invoiceNumber = `INV-SANDBOX-MIN-${suffix}`;
...
function createValidOrganizationNumber(seed: string): string {
  const base = `91${seed}`.replace(/\D/g, "").slice(0, 8).padEnd(8, "0");
  const weights = [3, 2, 7, 6, 5, 4, 3, 2];
  const sum = base.split("").reduce((acc, digit, index) => acc + Number(digit) * weights[index]!, 0);
  const remainder = sum % 11;
  const checkDigit = remainder === 0 ? 0 : 11 - remainder;
  if (checkDigit === 10) {
    return createValidOrganizationNumber(`${Number(seed) + 1}`);
  }
  return `${base}${checkDigit}`;
}

const organizationNumber = createValidOrganizationNumber(suffix);
...
async function main() {
  ...
  const supplierId = requireId(created.value?.id, "supplier.id");
  const supplierLedgerAccountId = requireId(created.value?.ledgerAccount?.id, "supplier.ledgerAccount.id");
  ...
  form.append("file", new Blob([buildInvoiceXml()], { type: "application/xml" }), `${invoiceNumber}.xml`);
  ...
  const voucherId = requireId(importedVoucher.id, "voucher.id");
  ...
  const postings = updated.value?.postings ?? [];
  ...
  const systemVatPosting = postings.find((posting) => posting.row !== 1 && posting.row !== 2 && posting.amount === vatAmount);

  if (!updated.value?.id || !expensePosting || !supplierPosting || !systemVatPosting) {
    throw new Error(`Write response did not prove final state: ${JSON.stringify(updated)}`);
  }
