...
const invoiceNumber = `INV-SANDBOX-7606-${Date.now()}`;
...
function chooseVatType(vatTypes: VatType[]): VatType {
  ...
  return matches.find((vatType) => String(vatType.number ?? "") === "1") ?? matches[0]!;
}
...
async function main() {
  ...
  form.append("description", `sandbox-verify-${invoiceNumber}`);
  form.append(
    "file",
  ...
  const supplierPosting = postings.find(
    (posting) =>
      posting.account?.id === supplier.supplierLedgerAccountId &&
  ...
  const vatPosting = postings.find(
    (posting) => posting.row !== 1 && posting.row !== 2 && posting.amount === vatAmount,
  );

  if (!voucher?.id || !expensePosting || !supplierPosting || !vatPosting) {
    throw new Error(`Final voucher state not proven: ${JSON.stringify(updated)}`);
  }
