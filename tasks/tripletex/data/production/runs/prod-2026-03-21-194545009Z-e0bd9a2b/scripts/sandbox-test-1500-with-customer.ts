...
async function main() {
  ...
  // Get a customer to use (the NOK test customer we created earlier, or any)
  const custRes = await get(`/customer?fields=id,name&count=3`);
  const customer = custRes.values?.[0];
  console.log(`Using customer: ID=${customer?.id}, name=${customer?.name}`);

  // Test: 8160/1500 with customer.id on the 1500 posting
  ...
  console.log(`\nVoucher 8160/1500 with customer SUCCEEDED: ID=${vRes.value?.id}`);
  const postings = vRes.value?.postings || [];
  for (const p of postings) {
    console.log(`  Row ${p.row}: account=${p.account?.number || p.account?.id}, amount=${p.amount}, customer=${p.customer?.id}`);
  }
}
