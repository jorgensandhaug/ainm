// Verify 5-step, 16-call path with frontloaded reads and voucher+invoice parallel
...
async function get(path: string) {
  ...
  if (!r.ok) { console.error(`[${n}] GET ${path} ${r.status}`, JSON.stringify(j)); throw new Error(`GET ${path} ${r.status}`); }
...
async function post(path: string, body: any) {
  ...
  const j = await r.json();
  if (!r.ok) { console.error(`[${n}] POST ${path} ${r.status}`, JSON.stringify(j)); throw new Error(`POST ${path} ${r.status}`); }
...
async function put(path: string, body: any) {
  ...
  const j = await r.json();
  if (!r.ok) { console.error(`[${n}] PUT ${path} ${r.status}`, JSON.stringify(j)); throw new Error(`PUT ${path} ${r.status}`); }
...
async function main() {
  ...
  // === STEP 1 (6 parallel): frontload ALL reads + customer create ===
  const [deptRes, custRes, pmRes, accRes, vtRes, vatRes] = await Promise.all([
    get("/department?isInactive=false&count=1&fields=*"),
    post("/customer", { name: `5Step Lifecycle ${ts}`, organizationNumber: "929610156", isCustomer: true }),
  ...
  console.log("Step 1 done: dept=", deptId, "cust=", customerId, "pm=", pmId);
  console.log("  acc1920=", acc1920.id, "bankNum=", acc1920.bankAccountNumber);

  // === STEP 2 (2-3 parallel): employees + project + optional bank fix ===
  const step2: Promise<any>[] = [
    post("/employee/list", [
      { firstName: "Emma", lastName: `Weber${ts}`, email: `emma5s.weber${ts}@example.org`, dateOfBirth: "1990-01-01", userType: "NO_ACCESS", ...(deptId ? { department: { id: deptId } } : {}) },
      { firstName: "Anna", lastName: `Becker${ts}`, email: `anna5s.becker${ts}@example.org`, dateOfBirth: "1992-06-15", userType: "NO_ACCESS", ...(deptId ? { department: { id: deptId } } : {}) },
    ]),
    post("/project", {
      name: `5Step Project ${ts}`,
      startDate: TODAY,
  ...
  if (bankNeedsFix) {
    step2.push(put(`/ledger/account/${acc1920.id}`, { id: acc1920.id, number: acc1920.number, name: acc1920.name, bankAccountNumber: "12345678903" }));
  }
  const step2Results = await Promise.all(step2);
  const empsRes = step2Results[0];
  const projRes = step2Results[1];
  const emp1Id = empsRes.values[0].id;
  const emp2Id = empsRes.values[1].id;
  const projectId = projRes.value.id;
  console.log("Step 2 done: emp1=", emp1Id, "emp2=", emp2Id, "proj=", projectId, "bankFix=", bankNeedsFix);

  // === STEP 3 (3 parallel): activity + 2 participants ===
  const totalHours = 73 + 134;
  const [paRes] = await Promise.all([
    post("/project/projectActivity", {
      project: { id: projectId }, startDate: TODAY, budgetHours: totalHours, budgetFeeCurrency: 405900,
  ...
  console.log("Step 4 done: supplier=", supplierId, "ts entries=", tsRes.values?.length, "orderline=", olRes.value.id);

  // === STEP 5 (2 parallel): voucher + invoice ===
  const [voucherRes, invoiceRes] = await Promise.all([
    post("/ledger/voucher", {
      date: TODAY, description: "Lieferantenkosten Silberberg GmbH", voucherType: { id: voucherTypeId },
      postings: [
    ...
    post("/invoice?sendToCustomer=false", {
      invoiceDate: TODAY, invoiceDueDate: "2026-04-22", customer: { id: customerId },
      orders: [{ customer: { id: customerId }, project: { id: projectId }, orderDate: TODAY, deliveryDate: TODAY,
        orderLines: [{ description: "5Step Lifecycle Project", count: 1, unitPriceExcludingVatCurrency: 405900, ...(vat25 ? { vatType: { id: vat25.id } } : {}) }],
      }],
    }),
  ]);
  ...
  console.log("Invoice amountExcludingVat:", invoiceRes.value.amountExcludingVatCurrency);
