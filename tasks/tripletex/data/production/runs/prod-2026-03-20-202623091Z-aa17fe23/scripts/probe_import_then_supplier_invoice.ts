...
const invoiceNumber = `INV-PROBE-IMP-${Date.now()}`;
...
type Voucher = { id?: number; version?: number; postings?: unknown[]; number?: number; date?: string };
...
if (!importResponse.ok) {
  throw new Error(`Import failed HTTP ${importResponse.status}: ${JSON.stringify(importData ?? importText)}`);
}
