...
const suffix = "321000010";
const supplierName = `Océan Reflection SARL ${suffix}`;
const organizationNumber = suffix;
const invoiceNumber = `INV-SANDBOX-4914-${suffix}`;
...
async function main() {
  ...
  const vatPosting = postings.find((posting) => posting.row !== 1 && posting.row !== 2 && posting.amount === vatAmount);
