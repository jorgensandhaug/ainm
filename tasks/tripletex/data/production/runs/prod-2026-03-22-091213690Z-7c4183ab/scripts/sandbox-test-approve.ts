...
const city = "Bergen";
const invoiceNumber = "INV-2026-8987-SBX4";
...
// Find the supplier invoice ID for this voucher
console.log("\n=== Find SI for this voucher ===");
const siListRes = await fetch(`${BASE}/supplierInvoice?supplierId=${supplierId}&invoiceDateFrom=2025-01-01&invoiceDateTo=2027-01-01&fields=id,invoiceNumber,voucher(id)`, {
  headers: { Authorization: AUTH }
});
const siList = await siListRes.json();
const si = siList.values.find((s: any) => s.voucher?.id === voucherId);
if (!si) { console.error("SI not found for voucher", voucherId); process.exit(1); }
const siId = si.id;
console.log("SI ID:", siId, "invoiceNumber:", si.invoiceNumber);
...
if (!approveRes.ok) {
  console.error("Approve failed:", JSON.stringify(approveData, null, 2));
} else {
  console.log("Approved SI:", JSON.stringify(approveData.value, null, 2));
}

// Book
console.log("\n=== Book ===");
// Need to re-GET the voucher version since approve may have changed it
const vGetRes = await fetch(`${BASE}/ledger/voucher/${voucherId}?fields=id,version`, {
  headers: { Authorization: AUTH }
});
const vGet = await vGetRes.json();
const versionForBook = vGet.value.version;
console.log("Version for book:", versionForBook);

const bookRes = await fetch(`${BASE}/ledger/voucher/${voucherId}?sendToLedger=true`, {
  method: "PUT", headers: H,
  body: JSON.stringify({
    version: versionForBook,
...
if (!bookRes.ok) {
  console.error("Book failed:", JSON.stringify(bookData, null, 2));
} else {
  console.log("BOOKED");
}

// Check approval status after booking
console.log("\n=== SI after booking ===");
const siAfter = await fetch(`${BASE}/supplierInvoice/${siId}?fields=*`, {
  headers: { Authorization: AUTH }
});
const siAfterData = await siAfter.json();
