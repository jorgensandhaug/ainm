...
async function api(method: string, path: string, body?: any): Promise<any> {
  ...
  if (res.status >= 400) console.log("  ERR:", typeof json === 'string' ? json.substring(0, 500) : JSON.stringify(json).substring(0, 500));
  return { status: res.status, data: json };
}

async function run() {
  // Use known-valid org 900969147 (from PDF). Search if supplier already exists.
  const orgNumber = "900969147";
  let supplierId: number;
  let ledgerAccountId: number;

  const { data: suppSearch } = await api("GET", `/supplier?organizationNumber=${orgNumber}&fields=id,name,organizationNumber,ledgerAccount(id)`);
  if (suppSearch.values?.length > 0) {
    supplierId = suppSearch.values[0].id;
    ledgerAccountId = suppSearch.values[0].ledgerAccount.id;
    console.log(`Existing supplier: id=${supplierId}`);
  } else {
    const { status, data } = await api("POST", "/supplier", {
      name: "Forêt SARL", organizationNumber: orgNumber,
      postalAddress: { addressLine1: "Nygata 51", postalCode: "0182", city: "Oslo", country: { id: 161 } },
      physicalAddress: { addressLine1: "Nygata 51", postalCode: "0182", city: "Oslo", country: { id: 161 } },
      bankAccountPresentation: [{ bban: "22757315878" }]
    });
    if (status !== 201) return;
    supplierId = data.value.id;
    ledgerAccountId = data.value.ledgerAccount.id;
  }
  console.log(`supplierId=${supplierId}, ledgerAcctId=${ledgerAccountId}`);
...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
  ...
  <cac:AccountingSupplierParty><cac:Party>
    ...
    <cac:PostalAddress>
      <cbc:StreetName>Nygata 51</cbc:StreetName><cbc:CityName>Oslo</cbc:CityName>
  ...
  if (siId) {
    ...
    console.log("Approve result:", appSt === 200 ? "OK" : JSON.stringify(appData).substring(0, 300));
  }
  ...
  const { status: bookSt } = await api("PUT", `/ledger/voucher/${voucherId}?sendToLedger=true`, {
    version: latestVersion, voucherType: { name: "Leverandørfaktura" }
  });
  ...
  // Compare with our WITHOUT-approval flow: check a previous SI
  console.log("\n=== Compare with non-approved SI ===");
  const { data: allSi } = await api("GET", "/supplierInvoice?invoiceDateFrom=2025-01-01&invoiceDateTo=2027-12-31&fields=id,invoiceNumber,approvalListElements&count=3&sorting=id&order=desc");
  for (const si of allSi.values || []) {
    console.log(`SI ${si.id} (${si.invoiceNumber}): approvalElements=${si.approvalListElements?.length}`);
  }
}
