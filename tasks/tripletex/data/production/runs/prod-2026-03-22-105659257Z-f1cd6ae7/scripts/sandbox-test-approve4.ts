...
async function api(method: string, path: string, body?: any): Promise<any> {
  ...
  if (res.status >= 400) console.log("  Error:", typeof json === 'string' ? json.substring(0, 300) : JSON.stringify(json).substring(0, 300));
  return { status: res.status, data: json };
}

async function run() {
  // Use an existing supplier from sandbox to avoid creation issues
  // First find one
  const { data: suppList } = await api("GET", "/supplier?count=1&fields=id,name,organizationNumber,ledgerAccount(id)");
  console.log("Existing supplier:", JSON.stringify(suppList.values?.[0]));

  // Create a NEW supplier with a unique but valid org number
  const rnd = Math.floor(Math.random() * 90000) + 10000;
  const orgNumber = `9001${rnd}`;
  const supplierName = `Approval Test ${rnd}`;
  const { status: suppStatus, data: suppData } = await api("POST", "/supplier", {
    name: supplierName, organizationNumber: orgNumber,
    postalAddress: { addressLine1: "Gate 1", postalCode: "0100", city: "Oslo", country: { id: 161 } },
    physicalAddress: { addressLine1: "Gate 1", postalCode: "0100", city: "Oslo", country: { id: 161 } },
  });
  if (suppStatus !== 201) return;
...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
  ...
  const { status: impStatus, data: imp } = await api("POST", "/ledger/voucher/importDocument", fd);
  if (impStatus !== 201) return;
  ...
  // Try APPROVE before booking
  if (siId) {
    console.log("\n=== APPROVE ===");
    const { status: appSt, data: appData } = await api("PUT", `/supplierInvoice/${siId}/:approve`);
    console.log("Approve result:", JSON.stringify(appData).substring(0, 300));
  }

  // Get fresh version
  const { data: freshVoucher } = await api("GET", `/ledger/voucher/${voucherId}?fields=id,version`);
  const latestVersion = freshVoucher.value.version;

  // Book
  const { status: bookSt, data: bookData } = await api("PUT", `/ledger/voucher/${voucherId}?sendToLedger=true`, {
  ...
  // Check approval elements detail
  if (siFinal.value?.approvalListElements?.length) {
    for (const elem of siFinal.value.approvalListElements) {
      ...
      console.log("ApprovalElement:", JSON.stringify(elemData.value, null, 2));
    }
  }
}
