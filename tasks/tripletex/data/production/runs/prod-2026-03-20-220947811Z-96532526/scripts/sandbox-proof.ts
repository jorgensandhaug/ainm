...
async function request<T>(
  ...
  if (!response.ok) throw new ApiError(response.status, body ?? text);
  return body as T;
}

function chooseManager(values: Employee[]): Employee {
  const withEmail = values.filter((employee) => employee.email);
  if (withEmail.length === 0) {
    throw new Error("No assignable project manager with email in sandbox");
  }
  return withEmail[0];
}
...
async function setupFixture() {
  ...
  } else if (project.projectManager?.id !== manager.id || Number(project.fixedprice) !== INITIAL_FIXED_PRICE) {
    project = (
...
async function main() {
  ...
  const exactProject = (projectSearch.values ?? []).find(
    (candidate) =>
      candidate.name === PROJECT_NAME &&
      candidate.customer?.organizationNumber === CUSTOMER_ORG_NO &&
      candidate.projectManager?.email === fixture.manager.email,
  );
  if (!exactProject?.id || !exactProject.customer?.id || !exactProject.projectManager?.id) {
    throw new Error(`Project-first proof failed: ${JSON.stringify(projectSearch.values ?? [])}`);
  }
    ...
    await request<ApiValue<Project>>(
      ...
      `project/${exactProject.id}`,
      {
        body: {
          ...
          projectManager: { id: exactProject.projectManager.id },
          isFixedPrice: true,
    ...
    await request<ApiValue<Order>>(
      ...
      "order",
      {
        body: {
          ...
          orderLines: [
            {
              description: "Sandbox proof 25 % partial billing",
              count: 1,
  ...
  console.log(
    JSON.stringify(
      {
        fixtureManagerEmail: fixture.manager.email,
        measuredCalls,
        measuredCallCount: measuredCalls.length,
        vatPercentage: vatType.percentage ?? null,
        updatedProjectFixedPrice: updatedProject.fixedprice ?? null,
