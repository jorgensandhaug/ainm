// Verify that buyer EndpointID 000000000 fails and 123456785 passes PEPPOL mod11
const BASE = "https://kkpqfuj-amager.tripletex.dev/v2";
const TOKEN = "eyJ0b2tlbklkIjoyMTQ3NjI4NDYxLCJ0b2tlbiI6IjU2NDRjNDljLWU0YzYtNGU2OC05MWU5LWU0MzBhMWRiYTMzYSJ9";
const AUTH = "Basic " + btoa("0:" + TOKEN);

async function tryImport(buyerOrgNum: string, label: string) {
  // Use an existing supplier in sandbox - Lumière SARL / 913175212
...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
  ...
  <cbc:ID>INV-SANDBOX-BUYER-TEST-${buyerOrgNum}</cbc:ID>
  ...
  const url = `${BASE}/ledger/voucher/importDocument`;
  ...
  try {
    ...
    if (res.ok) {
      console.log(`SUCCESS: voucher id=${json.values?.[0]?.id}`);
    } else {
      console.log(`FAILED: ${json.validationMessages?.map((v: any) => v.message).join('; ')}`);
    }
  } catch { console.log(text.slice(0, 500)); }
}

async function main() {
  // Test 1: 000000000 (expected to fail)
  await tryImport("000000000", "Test 1 - all zeros");

  // Test 2: 123456785 (expected to pass)
  await tryImport("123456785", "Test 2 - valid mod11");

  // Test 3: Also test if we can omit the buyer EndpointID entirely or use scheme "0007" to avoid the issue
  // Test with just a random valid org number like 979442454
  // Let me compute: 9*3+7*2+9*7+4*6+4*5+2*4+4*3+5*2 = 27+14+63+24+20+8+12+10 = 178, 178%11=2, check=9 → 979442459
  await tryImport("979442459", "Test 3 - another valid mod11");
}
