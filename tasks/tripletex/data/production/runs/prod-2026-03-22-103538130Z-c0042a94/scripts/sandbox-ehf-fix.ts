...
async function main() {
  // Get the company org number
  const whoAmI = await get("/token/session/>whoAmI?fields=company(organizationNumber)");
  const companyOrg = whoAmI?.value?.company?.organizationNumber || "971093866";
...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
  ...
  <cac:AccountingCustomerParty>
    <cac:Party>
      ...
      <cac:PartyName><cbc:Name>Test Company</cbc:Name></cac:PartyName>
      <cac:PostalAddress>
  ...
  console.log("Attempting importDocument...");
  ...
  if (r.ok) {
    ...
    console.log("\nSUCCESS! Voucher ID:", voucherId);

    // Check for supplier invoice entity
    const siRes = await get(`/supplierInvoice?voucherId=${voucherId}&fields=*`);
    if (siRes?.values?.length) {
      console.log("SI entity created:", JSON.stringify(siRes.values[0], null, 2).slice(0, 1500));
    } else {
      console.log("No SI entity found for voucher", voucherId);
    }
  }
}
