...
type PaymentTypeOut = { id: number; showWagePayment?: boolean; showWagePeriodTransaction?: boolean };
...
const variants = [
  { label: "baseline", query: "generateTaxDeduction=true" },
  { label: "completed-only", query: "generateTaxDeduction=true&completed=true" },
  { label: "complete-only", query: "generateTaxDeduction=true&complete=true" },
  {
    label: "paymentType-paymentDate",
    query: `generateTaxDeduction=true&paymentTypeId=${wagePaymentType.id}&paymentDate=${RUN_DATE}`,
  },
  {
    label: "completed-paymentType-paymentDate",
    query: `generateTaxDeduction=true&completed=true&paymentTypeId=${wagePaymentType.id}&paymentDate=${RUN_DATE}`,
  },
  {
    label: "complete-paymentType-paymentDate",
    query: `generateTaxDeduction=true&complete=true&paymentTypeId=${wagePaymentType.id}&paymentDate=${RUN_DATE}`,
  },
  {
    label: "book-paymentType-paymentDate",
    query: `generateTaxDeduction=true&book=true&paymentTypeId=${wagePaymentType.id}&paymentDate=${RUN_DATE}`,
  },
  {
    label: "deliver-paymentType-paymentDate",
    query: `generateTaxDeduction=true&deliver=true&paymentTypeId=${wagePaymentType.id}&paymentDate=${RUN_DATE}`,
  },
  {
    label: "sendToBank-false-paymentType-paymentDate",
    query: `generateTaxDeduction=true&sendToBank=false&paymentTypeId=${wagePaymentType.id}&paymentDate=${RUN_DATE}`,
  },
];

const results = [];
for (const variant of variants) {
  const created = await maybeApi(`salary/transaction?${variant.query}`, {
    method: "POST",
    body: JSON.stringify(payload),
  });
  if (!("value" in (created as Record<string, unknown>))) {
    results.push({ label: variant.label, query: variant.query, create: created });
    continue;
  }

  const createdValue = created as WrappedValue<{ id: number; payslips?: Array<{ id?: number }> }>;
  const transactionId = createdValue.value.id;
  const transaction = await api<WrappedValue<Record<string, unknown>>>(`salary/transaction/${transactionId}?fields=*`);
  const payslipId =
  ...
  const payslip =
    payslipId === null
      ? null
      : await maybeApi(`salary/payslip/${payslipId}?fields=*,specifications(*,salaryType(*))`);
...
console.log(JSON.stringify({ wagePaymentType, results }, null, 2));
