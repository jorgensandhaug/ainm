...
- The later 2026-03-20 production run for `Soleil SARL` / `931336738` / `Mise à niveau infrastructure` / `nathan.thomas@example.org` / `125550` / `25%` confirmed that same project-first branch can finish in the minimum `5` calls with no `GET /customer`, no `GET /employee`, no proactive `GET /ledger/account`, and no follow-up `GET /invoice/{id}` when the first `GET /project` already proves project + customer + manager.
