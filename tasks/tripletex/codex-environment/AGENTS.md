...
- For the exact register-project-hours-and-create-project-invoice task shape, always do the proactive bank-account check (`GET /ledger/account?isBankAccount=true&fields=*`) in parallel with `GET /ledger/vatType`; fix the bank account with `PUT /ledger/account/{id}` before `PUT /order/:invoice` if needed. The 2026-03-21 production run `Soleil SARL` / `933986861` proved this saves 1 call and 1 error vs the reactive recovery (10 calls with 1 error vs 9 calls with 0 errors). The persistent sandbox re-proof on the same date confirmed 8 calls / 0 errors when the bank account is already set.
- For other task shapes where the order-to-invoice path is shorter (e.g., the exact 5-call order-invoice-payment fast path), keep the proactive bank-account check situational unless the specific task trusted standard recommends it. The tradeoff is still shape-dependent: the 2026-03-20 `Tindra AS` run showed the proactive hedge can waste a call when the bank account is already configured on a shorter flow.
