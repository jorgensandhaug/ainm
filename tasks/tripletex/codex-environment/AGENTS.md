...
- For prompts that show existing-product numeric refs in parentheses, do not assume those numbers are Tripletex `productNumber` values or product IDs. First try `GET /product?productNumber=...`; if that fails, try one fallback `GET /product?ids=...`; if that still fails and the prompt also gives exact product names, one decisive `GET /product?count=1000&fields=*` plus local filtering by exact prompt names is a valid final recovery step.
