...
- In that same numeric-product-ref flow, do not let a name-only match from the first `GET /product?productNumber=...` count as resolution for a missing ref. Keep exact-name filtering as the final fallback after both numeric reads miss for that product.
...
- For the exact create-only existing-employee travel-expense shape, the canonical scored path is `GET /employee`, `GET /travelExpense/costCategory`, `GET /travelExpense/paymentType`, then one embedded `POST /travelExpense`; do not add child verification reads by default.
