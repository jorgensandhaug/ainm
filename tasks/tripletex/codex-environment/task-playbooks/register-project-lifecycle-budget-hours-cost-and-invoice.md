...
- `POST /order` → `PUT /order/:invoice` (isApproved=true); fallback `POST /invoice` with embedded `orders[]` if proxy rejects `:invoice` action path
- importDocument creates real `supplierInvoice` entity (sandbox-verified 2026-03-22, potential check 5 fix — awaiting production validation)
- Diagnostic GETs for full state logging

## Optimal Path

**17-18 writes + free diagnostic GETs, 0 errors, 9 sequential steps.** (17 writes base; 18 if bank account needs repair — conditional PUT.) GETs are free. Includes voucher for check 6 + hourly rates for check 4 + importDocument for check 5. Sandbox-verified 2026-03-22, **production-confirmed 2026-03-22** (run 4db584f1, `Cloud Migration Northwave`, 0 errors, hourlyRate=2496, chargeable=true, invoice 396900 ex-VAT / 496125 incl-VAT, isApproved=true — note: 4db584f1 did NOT include importDocument; importDocument is a NEW addition sandbox-verified but awaiting production validation).

Steps: (1) 6 GETs + POST customer; (2) POST employees + POST project (NO isFixedPrice); (3) POST activity (isChargeable:true) + POST participants; (4) GET hourlyRates holder → PUT model → 2× POST specificRates; (5) POST timesheet + POST supplier + POST orderline; (6+7) POST voucher + POST order; (6B) POST importDocument → PUT postings → PUT book; (8) PUT order/:invoice.

**importDocument note:** Creates a real `supplierInvoice` entity via EHF XML. The standard voucher (step 6) provides check 6; importDocument provides check 5. Uses `buildEhfXml()` with PaymentMeans+PaymentID. Adds 3 writes (import + PUT postings sendToLedger=false + PUT book sendToLedger=true). If import fails, log and continue — the standard voucher still works.
