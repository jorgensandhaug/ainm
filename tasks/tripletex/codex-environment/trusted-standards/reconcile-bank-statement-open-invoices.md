...
**ALL prior completed runs scored 0.6/6.** The full flow (opening balance + bank import + multi-period matching + close) was sandbox-verified END-TO-END on 2026-03-22. Pre-built script v3 (with invoice reference matching + batch matching + combined voucher) first production-tested in French run b8a43ac0 (13 mutating, 0 errors, score pending).

- **French run (b8a43ac0): 13 mutating calls, 0 errors, scored pending** — FIRST v3 script production run. 10 CSV lines, 2 periods (Jan+Feb). 5 customer payments (4 full + 1 partial: Bernard SARL 1975/4937.5), 3 suppliers, 2 non-invoice (Renteinntekter Ut + Skattetrekk Ut). All 10 matched via batch matching (2 batches). Both recons closed (159250 + 148014.53). Invoice reference matching via modulo worked perfectly. Copied pre-built script, ran in ~15s.
