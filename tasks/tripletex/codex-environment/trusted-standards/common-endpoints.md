...
- Standard explicit-VAT note:
  ...
  - further re-confirmed on 2026-03-21 for `Elvdal AS` / `810713909` / `Nettverksteneste (7765)` + `Konsulenttimar (4369)` + `Vedlikehald (5331)`: `GET /product?productNumber=7765&productNumber=4369&productNumber=5331&fields=*` returned only product `5331`, wasting 1 call before the catalog read found all three; when the prompt gives both exact names and parenthetical numbers, the catalog read is always the right first choice because it is never more calls and avoids the partial-result risk
