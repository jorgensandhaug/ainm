...
- Standard create prerequisites:
  ...
  - often product ids
- Standard fast-path note:
  - for exact existing-customer plus existing-product order-to-invoice-to-payment tasks, prefer `./trusted-standards/create-order-invoice-and-register-payment.md`; the winning path is now `POST /invoice` with embedded `orders[]` (creates order + invoice + payment in one call), with a proactive bank-account hedge: customer read → product read (with `vatType(*)`) → payment-type read → bank-account check (conditional repair) → `POST /invoice`
  ...
  - for the order-to-invoice-to-payment task shape on fresh accounts, the proactive bank-account hedge (`GET /ledger/account?isBankAccount=true&fields=*` before the invoice write) is now the recommended default; the 2026-03-22 production run proved that omitting the hedge and then hitting the 422 causes catastrophic script-restart waste (11 calls instead of 6); the hedge costs 1 extra call when bank acct exists (5 total) but saves 1 call + avoids 1 error when missing (6 vs 7 + 1 error)
...
- Standard payment note:
  - for the exact order-to-invoice-to-full-payment task shape, the canonical 4-call path uses `POST /invoice` with embedded `orders[]` and `paymentTypeId` + `paidAmount` query params; `paidAmount` must be the exact invoice total including VAT, computed as `Σ(unitPrice × count × (1 + vatType.percentage/100))` from the product lookup (which must include `vatType(*)` in fields)
  - do NOT use `paidAmount=0.01` with `POST /invoice` — there is no `paymentTypeIdRestAmount` parameter on `POST /invoice`, so it only pays the literal `paidAmount`; overpaying creates negative outstanding (credit); `paidAmount=0` is rejected as missing
  ...
  - the old 5-call path (`POST /order` + `PUT /order/:invoice` with `paymentTypeIdRestAmount`) is superseded by the 4-call `POST /invoice` path; sandbox-verified 2026-03-22
...
- Standard create note:
  ...
  - for supplier-invoice tasks (T11 text-only and T20 from-PDF), ALWAYS use `POST /ledger/voucher/importDocument` with EHF XML — it creates the required `supplierInvoice` entity; direct `POST /ledger/voucher` does NOT create a supplierInvoice and scored 0-1/8; see `./register-supplier-invoice.md` and `./register-supplier-invoice-from-pdf.md`
  - `POST /ledger/voucher/importDocument` returns a **list wrapper** `{ values: [{ id, version }] }`, NOT `{ value: { id } }` — extract from `response.values[0]`; all other voucher endpoints (PUT) return `{ value: {...} }` (singular); using `.value.id` on importDocument response will crash
  - `POST /supplier` response includes `value.ledgerAccount.id` which IS account 2400's id — do NOT waste a separate GET for it
