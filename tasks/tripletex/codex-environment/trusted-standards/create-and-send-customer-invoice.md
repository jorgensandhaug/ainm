...
3b. **DO NOT do a proactive bank-account check** — skip `GET /ledger/account` entirely; the proactive approach costs 1 extra write (PUT) which drops efficiency from 2.0 to 1.5; instead, go optimistic: POST /invoice directly and only repair if it fails
...
5. only if the invoice write fails with `422` mentioning `bankkontonummer` (missing company bank account):
   ...
   - this reactive path costs 2 writes + 1 error (worse than the optimistic happy path of 1 write) but is unavoidable when bank is empty
6. stop
...
## Verification
GETs are free from scoring — only writes (POST/PUT/DELETE) and errors count against efficiency. The 2026-03-22 Bergvik run scored 1.5/2.0 because of 2 writes (PUT bank + POST invoice), not because of GETs. Verification GETs are fine for logging. The key optimization is the optimistic bank approach (skip the proactive PUT bank write).
...
- when the prompt gives product numbers, the optimal call count depends on whether the customer is new or existing (vatType IDs are hardcoded — no GET /ledger/vatType needed):
  - **new customer** (fresh account): 3 writes (2 parallel [POST /customer + POST /product/list] + 1 POST /invoice) + free GETs, or +1 write with bank repair
  - **existing customer** with existing products: 1 write (POST /invoice) + free GETs [GET /customer + GET /product + GET /ledger/account], or +1 write with bank repair
  - **existing customer** with missing products: 2 writes (POST /product/list + POST /invoice) + free GETs, or +1 write with bank repair
...
- preemptive bank-account resolution tested in persistent sandbox on 2026-03-21:
  ...
  - **REVERSED AGAIN 2026-03-22**: proactive bank check guarantees 2 writes → 1.5/2.0 efficiency ceiling; optimistic approach (skip bank check, POST /invoice directly) gives 1 write when bank is set → 2.0/2.0; since best_score is lifetime, the optimistic approach is now recommended; reactive bank repair (on 422) is the fallback
...
- existing-customer direct-line create-and-send confirmed in production on 2026-03-21:
  ...
  - the 2026-03-22 English production run `Blueshore Ltd` / `987928921` / `Maintenance` / `40600` / `excluding VAT` confirmed the existing-customer + bank-repair shape in 6 calls (reactive) with 1 avoidable 422: `GET /customer` → `POST /invoice` (422 bank) → `GET /ledger/account` → `PUT /ledger/account/{id}` → `POST /invoice` (201, `amountExcludingVatCurrency=40600`, `amountCurrency=50750`); the reactive bank repair is correct (optimistic approach) — it allows 1-write path when bank is already set
  - the 2026-03-22 Bergvik AS production run scored 1.5/2.0 with proactive bank check (2 writes: PUT bank + POST invoice); to reach 2.0, the optimistic approach (POST /invoice directly, no bank check) must hit an account where bank is already set (1 write only)
  - sandbox re-confirmed that omitting `vatType` on a direct line creates 0% VAT (amountCurrency == amountExcludingVatCurrency), proving hardcoded vatType IDs are essential for the taxed branch
  - the 2026-03-22 Spanish production run `Solmar SL` / `893298169` / `Mantenimiento` / `19500` / `sin IVA` confirmed hardcoded vatType.id=6 for the 0% Spanish branch: `POST /customer` → `POST /invoice` with `vatType: { id: 6 }` (201, `amountExcludingVatCurrency=19500`, `amountCurrency=19500`)
