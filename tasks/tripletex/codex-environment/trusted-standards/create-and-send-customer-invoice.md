...
- the 2026-03-21 production run for `Bergvik AS` / `890733751` / `Systemutvikling` / `28900` / `eksklusiv MVA` used the wrong flow entirely — the agent selected the order-based standard and spent 8 calls; the 2026-03-22 re-run of the SAME task proved the optimal 4-call path (2 free GETs + 2 writes + 0 errors): `GET /customer` (parallel with `GET /ledger/account`) → `PUT /ledger/account/{id}` (bank repair) → `POST /invoice` with hardcoded `vatType: { id: 3 }` (201, `amountExcludingVatCurrency=28900`, `amountCurrency=36125`); this beats the old 6-call reactive path by: hardcoding vatType.id=3 (saves 1 GET) + proactive bank check (saves 1 failed POST + 1 retry POST); sandbox-verified 2026-03-22
...
- existing-customer direct-line create-and-send confirmed in production on 2026-03-21:
  ...
  - the 2026-03-22 English production run `Blueshore Ltd` / `987928921` / `Maintenance` / `40600` / `excluding VAT` confirmed the existing-customer + bank-repair shape in 6 calls (reactive) and 1 avoidable 422: `GET /customer` (parallel with `GET /ledger/vatType`, found vatType.id=3 at 25%) → `POST /invoice` (422 bank) → `GET /ledger/account` (found account 1920, id=498367178) → `PUT /ledger/account/498367178` (registered `12345678903`) → `POST /invoice` (201, `amountExcludingVatCurrency=40600`, `amountCurrency=50750`); third English definite-article existing-customer confirmation; **post-run finding**: proactive bank-account check (free GET before POST) would have reduced to 5 calls, 2 writes, 0 errors instead of 6 calls, 3 writes, 1 error; trusted standard updated to recommend proactive approach
  - persistent sandbox re-verification on 2026-03-22 confirmed the proactive 3-way parallel approach [GET /customer + GET /ledger/vatType + GET /ledger/account] + POST /invoice succeeds with 0 errors in both happy-case and repair-needed scenarios; sandbox now has full VAT types (3/25%, 31/15%, 32/12%, 5/0%, 52/0%, 6/0%)
  ...
  - the 2026-03-22 Spanish production run `Solmar SL` / `893298169` / `Mantenimiento` / `19500` / `sin IVA` confirmed the optimal new-customer + hardcoded vatType.id=6 + proactive bank check path in 4 calls, 0 errors: `POST /customer` (parallel with `GET /ledger/account`) → `PUT /ledger/account/{id}` → `POST /invoice` with `vatType: { id: 6 }` (201, `amountExcludingVatCurrency=19500`, `amountCurrency=19500`); this is the first production confirmation of hardcoded vatType for the 0% Spanish branch and the first new-customer proactive bank check production confirmation; sandbox re-verified same amounts and `vatType.percentage=0` on readback
