...
- persistent sandbox proved:
  ...
  - persistent-sandbox re-proof on 2026-03-22 confirmed: `GET /activity/>forTimeSheet` works WITHOUT `employeeId` parameter — returns the same activities; however, `projectId` is still required, so the activity GET cannot be moved to step 1 (it depends on the project GET result); `employeeId` being optional does not enable any call reduction
  - the 2026-03-22 production Norwegian run `Nordhav AS` / `912074005` / `Skytjeneste-oppsett` / `silje.strand@example.org` / `Design` / `5` hours / `1750` (dcf3ce59) matched the non-chargeable branch with the optimized 3-step layout, completed in `8` calls (3 writes + 5 reads, with bank fix) and `0` errors, returned `amountExcludingVatCurrency=8750` plus `amountCurrencyOutstanding=10937.5` (25% VAT); this is the 3rd consecutive run using the optimized 3-step layout with 0 errors, confirming the path is stable; the 8-call floor (7 configured / 8 unconfigured) is mathematically minimal — 5 reads to resolve IDs + 2-3 writes = 7-8 calls, no lower-call path exists
...
- the 2026-03-21 production German run `Sonnental GmbH` / `839389701` / `Datenmigration` / `paul.muller@example.org` / `Testing` / `33` hours / `900` completed in `12` calls with `0` errors (11 base + 1 bank fix); invoice returned `amountExcludingVatCurrency=29700` with `projectInvoiceDetails.length=1`; however, the agent used 6 sequential steps instead of the optimal 4 — vatType and account reads were placed in step 4 instead of step 1, and invoice was sequential after timesheet instead of parallel; same-day sandbox re-proof confirmed the 4-step layout (11 calls, 0 errors) and the 10-call path without participant (10 calls, 0 errors); the agent also used `adminAccess: true` instead of the standard's `adminAccess: false` — this task shape does NOT designate the employee as project manager, so `false` is correct

### Create From Scratch Avoidable Mistakes
- Do not place `GET /ledger/vatType` and `GET /ledger/account` in a later step — they have no dependencies; they MUST be in step 1 (parallel with dept, customer, PM) to enable timesheet + invoice to run in parallel at step 4; placing them later adds 2 unnecessary sequential steps (6 steps instead of 4)
- Do not use `adminAccess: true` on the participant unless the prompt designates the employee as project manager — this simpler task shape has no PM designation; use `adminAccess: false`
- Do not confuse this task shape with the lifecycle standard (`register-project-lifecycle-budget-hours-cost-and-invoice`) — the lifecycle standard requires 2 employees, supplier cost, and budget; this shape has 1 employee, no supplier cost, and the budget is simply hours × rate
- Do not split hours <= 24 across multiple entries — a single entry up to 24h works fine in `POST /timesheet/entry/list`
- Do not run invoice after timesheet sequentially — the invoice does NOT depend on timesheet entries existing; both depend only on project + activity from step 3 and can run in parallel
