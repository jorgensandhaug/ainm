...
- division remains at 0% in production (0/15 runs); pre-reading it would waste 1 call every time
...
- out of 16 known production create-employee runs, 4 succeeded without department and 9 needed it (3 used pre-read so requirement is indeterminate); dept-required rate is at least 56%
...
- Edward Harris run (20daf6b5, English prompt, 1987-11-09, edward.harris@example.org, start 2026-07-06) used the pre-read strategy correctly; 2 calls, 0 errors (GET /department found 745977, POST /employee with dept→201); 3rd production run achieving the proven-minimum 2-call path; 15th create-employee run overall
- Solveig Johansen run (de91aeca, Norwegian Bokmål prompt, 1993-05-15, solveig.johansen@example.org, start 2026-01-18) used the pre-read strategy correctly; 2 calls, 0 errors (GET /department found 1020861, POST /employee with dept→201); 4th production run achieving the proven-minimum 2-call path; 16th create-employee run overall
