...
- for the exact prompt shape `organizationNumber=829535181`, `description="Nettverksteneste"`, `amountExcludingVatCurrency=15550`, that two-call path was the successful production path on 2026-03-22 (Norwegian nynorsk prompt); plus 2 free verification GETs for logging
...
- production run re-verified on 2026-03-21 for the exact prompt shape `organizationNumber=911680521`, `description="Systemutvikling"`, `amountExcludingVatCurrency=8050`:
  - `GET /invoice?invoiceDateFrom=2000-01-01&invoiceDateTo=2026-03-22&count=1000&sorting=-invoiceDate&fields=*,customer(*),orderLines(*),orders(*,orderLines(*))`
...
- production run re-confirmed on 2026-03-21 for the exact prompt shape `organizationNumber=991882502`, `description="Opplæring"`, `amountExcludingVatCurrency=13100`:
  - `GET /invoice?invoiceDateFrom=2000-01-01&invoiceDateTo=2026-03-22&count=1000&sorting=-invoiceDate&fields=*,customer(*),orderLines(*),orders(*,orderLines(*))`
...
- production run confirmed on 2026-03-21 for the exact prompt shape `organizationNumber=962467210`, `description="Nettverkstjeneste"`, `amountExcludingVatCurrency=41600`:
  - `GET /invoice?invoiceDateFrom=2000-01-01&invoiceDateTo=2026-03-22&count=1000&sorting=-invoiceDate&fields=*,customer(*),orderLines(*),orders(*,orderLines(*))`
  - `PUT /invoice/2147644728/:createCreditNote?date=2026-03-21&sendToCustomer=false`
  - the run succeeded with 2 API calls, 0 errors
  - re-verified in persistent sandbox on 2026-03-21 with a disposable fixture matching `organizationNumber=962467210`, `description="Nettverkstjeneste"`, `amountExcludingVatCurrency=41600`; the same two-call core located the invoice and created the credit note with `isCreditNote=true` and `creditedInvoice=<original id>` and no follow-up read
- production run confirmed on 2026-03-22 for the exact prompt shape `organizationNumber=978503071`, `description="Licencia de software"`, `amountExcludingVatCurrency=25450` (Spanish prompt):
...
- production run confirmed on 2026-03-22 for the exact prompt shape `organizationNumber=871338140`, `description="Diseño web"`, `amountExcludingVatCurrency=39850` (Spanish prompt):
  - `GET /invoice?invoiceDateFrom=2000-01-01&invoiceDateTo=2026-03-23&count=1000&sorting=-invoiceDate&fields=*,customer(*),orderLines(*),orders(*,orderLines(*))`
  - `PUT /invoice/2147664097/:createCreditNote?date=2026-03-22&sendToCustomer=false`
  - the run succeeded with 2 API calls, 0 errors
  - re-verified in persistent sandbox on 2026-03-22 with a disposable fixture matching `organizationNumber=871338140`, `description="Diseño web"`, `amountExcludingVatCurrency=39850`; the same two-call core located the invoice and created the credit note with `isCreditNote=true` and `creditedInvoice=<original id>` and no follow-up read
