...
- production create on `2026-03-21` for `Migrasjon Elvdal` / `Elvdal AS` / `877501906` plus `liv.haugen@example.org` also succeeded with the same exact `3`-call path and no follow-up reads; third Nynorsk prompt handled identically, 13 consecutive optimal runs confirm the standard is stable
- production create on `2026-03-21` for `Analyse Sonnental` / `Sonnental GmbH` / `980215350` plus `emma.schneider@example.org` also succeeded with the same exact `3`-call path and no follow-up reads; second German prompt handled identically, 14 consecutive optimal runs across en/pt/es/nb/nn/fr/de confirm the standard is fully language-independent and stable
- persistent sandbox re-proof on `2026-03-21` re-confirmed that nested customer `{ organizationNumber }` without `name` returns `422 customer.name: Kan ikke være null.`, and nested manager `{ firstName, lastName, email }` without `id` still returns `422 Feltet "Prosjektleder" må fylles ut.`; the `3`-call minimum is unchanged
- production create on `2026-03-21` for `Implementering Nordhav` / `Nordhav AS` / `957080138` plus `silje.degard@example.org` (e3fd9f40) also succeeded with the same exact `3`-call path and no follow-up reads; 15 consecutive optimal runs confirm the standard is stable
- production create on `2026-03-21` for `Implementering Strandvik` / `Strandvik AS` / `935092957` plus `hakon.berge@example.org` (dde40565) also succeeded with the same exact `3`-call path and no follow-up reads; fourth Nynorsk prompt handled identically, 16 consecutive optimal runs across en/pt/es/nb/nn/fr/de confirm the standard is fully language-independent and stable
- persistent sandbox re-proof on `2026-03-22` re-confirmed that nested customer `{ name, organizationNumber }` returns `422`, nested PM `{ email, firstName, lastName }` without `id` returns `422 Feltet "Prosjektleder" må fylles ut.`; the `3`-call minimum is unchanged
- production create on `2026-03-22` for `Migrasjon Elvdal` / `Elvdal AS` / `962211348` plus `geir.aasen@example.org` (3d670cac) also succeeded with the same exact `3`-call path and no follow-up reads; fifth Nynorsk prompt handled identically, 17 consecutive optimal runs across en/pt/es/nb/nn/fr/de confirm the standard is fully language-independent and stable
- persistent sandbox re-proof on `2026-03-22` re-confirmed that inline customer `{ name, organizationNumber }` on POST /project returns `201` but `customer=null` (silently dropped), inline PM `{ email }` without `id` returns `422` (firstName/lastName required), and real customer ID + inline PM email-only returns `422`; the `3`-call minimum is unchanged
- production create on `2026-03-22` for `Analyse Sjøbris` / `Sjøbris AS` / `883693329` plus `steinar.berge@example.org` (eb378aba) also succeeded with the same exact `3`-call path and no follow-up reads; fifth Nynorsk prompt (`Prosjektleiar`, `knytt til`) handled identically, 18 consecutive optimal runs across en/pt/es/nb/nn/fr/de confirm the standard is fully language-independent and stable
- production create on `2026-03-22` for `Implementation Ridgepoint` / `Ridgepoint Ltd` / `948050927` plus `edward.brown@example.org` (e8933369) also succeeded with the same exact `3`-call path and no follow-up reads; English prompt handled identically, 19 consecutive optimal runs across en/pt/es/nb/nn/fr/de confirm the standard is fully language-independent and stable
- production create on `2026-03-22` for `Implementering Tindra` / `Tindra AS` / `886715536` plus `jonas.haugen@example.org` (cf7bf31b) also succeeded with the same exact `3`-call path and no follow-up reads; Bokmål prompt handled identically, 20 consecutive optimal runs across en/pt/es/nb/nn/fr/de confirm the standard is fully language-independent and stable
- production create on `2026-03-22` for `Oppgradering Fjordkraft` / `Fjordkraft AS` / `953177234` plus `henrik.johansen@example.org` (79b78ed4) also succeeded with the same exact `3`-call path plus 1 free verification GET; Bokmål prompt handled identically, 21 consecutive optimal runs across en/pt/es/nb/nn/fr/de confirm the standard is fully language-independent and stable
- production create on `2026-03-22` for `Implementering Tindra` / `Tindra AS` / `886715536` plus `jonas.haugen@example.org` (35fd492c) also succeeded with the same exact `3`-call path plus 1 free verification GET; Bokmål prompt handled identically, 22 consecutive optimal runs across en/pt/es/nb/nn/fr/de confirm the standard is fully language-independent and stable
