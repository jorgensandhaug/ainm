...
export const strategy = {
  strategyId: "15.register-hours-direct-invoice.v2",
  strategyPath:
    "src/tasks/task-15/strategies/register-hours-direct-invoice.ts",
  taskId: REGISTER_PROJECT_HOURS_AND_CREATE_PROJECT_INVOICE_TASK_ID,
  name: "Register hours then direct invoice",
  summary:
    "Resolves the employee, project, and project activity, writes the required project hours, then creates an unsent project-linked invoice directly via POST /invoice with an embedded order.",
  hypothesis:
    "POST /invoice with an embedded order and order lines replaces the two-step POST /order + PUT /order/:invoice path, saving one API call on every branch.",
  expectedCallProfile: {
    targetCalls: 6,
    maxCalls: 11,
  },
  stepOutline: [
    ...
    "POST /invoice with one embedded order containing a real project-linked order line derived from the prompt hours and rate.",
  ],
  ...
  async run(
    ...
    try {
      invoiceResponse = await createDirectInvoice(ctx, {
        customerId,
        projectId: project.id,
        invoiceDate,
        invoiceDueDate,
    ...
    const orderId = requireId(invoice.orders?.[0]?.id, "embedded order");
    ...
    if (activity.isChargeable !== true) {
      notes.push(
        "Resolved activity is non-chargeable, so the strategy skipped project-hourly-rate writes and used the direct invoice path with an embedded order.",
      );
    }
...
async function createDirectInvoice(
  ctx: StrategyContext,
  params: DirectInvoiceParams,
): Promise<ResponseWrapper<InvoiceSummary>> {
  return ctx.tripletex.post<ResponseWrapper<InvoiceSummary>>(
    "/invoice",
    {
      body: {
        ...
        orders: [
          {
            ...
            orderDate: params.invoiceDate,
            deliveryDate: params.invoiceDate,
            invoiceOnAccountVatHigh: false,
            orderLines: [
              {
                description: params.invoiceLineDescription,
                count: params.hours,
                unitPriceExcludingVatCurrency: params.hourlyRate,
                vatType: { id: params.vatTypeId },
              },
            ],
          },
        ],
