...
export const strategy = {
  strategyId: "20.register-supplier-invoice-pdf.v1",
  strategyPath:
    "src/tasks/task-20/strategies/register-supplier-invoice-pdf.ts",
  taskId: REGISTER_SUPPLIER_INVOICE_PDF_TASK_ID,
  name: "Import XML, book voucher, and attach source PDF",
  summary:
    "Creates or resolves the supplier, imports a valid XML supplier invoice, books the final taxable postings, then uploads the original PDF attachment to the voucher.",
  hypothesis:
    "The reliable supplier-invoice path still starts with XML import plus voucher update, but task 20 additionally requires preserving the original PDF by uploading it to the created voucher attachment endpoint.",
  expectedCallProfile: {
    targetCalls: 6,
    maxCalls: 7,
  },
  stepOutline: [
    ...
    "API call 5: PUT /ledger/voucher/{id}?sendToLedger=false with only version and postings.",
    "API call 6: POST /ledger/voucher/{id}/attachment with the original PDF attachment from the request.",
