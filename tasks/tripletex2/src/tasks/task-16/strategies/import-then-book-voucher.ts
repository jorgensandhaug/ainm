...
export const strategy = {
  strategyId: "16.import-then-book-voucher.v1",
  strategyPath:
    "src/tasks/task-16/strategies/import-then-book-voucher.ts",
  taskId: REGISTER_SUPPLIER_INVOICE_TASK_ID,
  name: "Import XML then book voucher",
  summary:
    "Creates or resolves the supplier, imports a valid EHF/UBL invoice document, then books the supplier invoice through a partial voucher update with explicit VAT and supplier postings.",
  hypothesis:
    "The trusted supplier-invoice path is XML import first, then a partial voucher update that keeps the imported header immutable while adding the exact accounting postings.",
  expectedCallProfile: {
    targetCalls: 5,
    maxCalls: 6,
  },
